const CACHE='cuaderno-ef-v3-9-11';
const ASSETS=['./','./index.html','./styles-v3911.css?v=3.9.11','./app-v3911.js?v=3.9.11','./manifest.webmanifest?v=3.9.11','./icons/icon-192-v382.png','./icons/icon-512-v382.png','./icons/apple-touch-icon-v382.png'];
self.addEventListener('install',e=>e.waitUntil(caches.open(CACHE).then(c=>c.addAll(ASSETS)).then(()=>self.skipWaiting())));
self.addEventListener('activate',e=>e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim())));
self.addEventListener('fetch',e=>{
 if(e.request.method!=='GET') return;
 const u=new URL(e.request.url);
 const isCore=u.pathname.endsWith('/index.html')||u.pathname.endsWith('/styles-v3911.css')||u.pathname.endsWith('/app-v3911.js')||u.pathname.endsWith('/cuaderno_ef/');
