...
const compMatrix={'1.5':['UP2'],'1.6':['UP2','UP6'],'2.1':['UP4'],'3.1':['UP3','UP5','UP7','UP8'],'3.2':['UP2','UP3','UP4'],'3.3':['UP1','UP2','UP3'],'4.1':['UP8']};
...
function defaultState(){return {groups:[],scores:{},rubricMarks:{},qual:{},qualDates:{},continuousClosures:{},attendance:{},notes:{},teacherNotes:{},fitness:{},reflections:{},warmups:{},up3RPE:{},up3Tasks:{},up4Game:{},photos:{},selectedGroup:null,selectedUP:'UP1',selectedDate:todayISO(),evalConfig:{upPeriods:defaultUpPeriods()},ui:{scoreMode:'numeric',evaluationPeriod:'eval1',openWarmup:null,openUp3RPE:null,openUp3Task:null,openUp4Game:null,openSummaryStudent:null,openEvaluateStudent:null}}}
const MATRIX_SCHEMA_V3911='matrix-source-v3911';
function migrateToMatrixV3911(s){
 if(s.matrixSchema===MATRIX_SCHEMA_V3911)return s;
 const oldEvidence={
  '1.1':['UP1','UP3'],'1.2':['UP2','UP3','UP6'],'1.3':['UP6'],'1.4':['UP6'],'1.5':['UP2','UP6'],'1.6':['UP1','UP2','UP3','UP6'],
  '2.1':['UP4','UP11'],'2.2':['UP4','UP9','UP11'],'2.3':['UP4','UP5','UP7','UP8','UP9','UP10'],'2.4':['UP4','UP9'],
  '3.1':[],'3.2':[],'3.3':[],'4.1':['UP8','UP10','UP12'],'4.2':['UP9','UP12'],'4.3':['UP10'],'5.1':['UP13'],'5.2':['UP13'],'5.3':['UP13']
 };
 const oldScores={...s.scores},oldMarks={...s.rubricMarks},oldQual={...s.qual},oldDates={...s.qualDates};
 const newScores={...s.scores},newMarks={...s.rubricMarks},newQual={...s.qual},newDates={...s.qualDates};
 const desiredQual=new Set();Object.entries(compMatrix).forEach(([c,arr])=>arr.forEach(up=>desiredQual.add(`${c}|${up}`)));
 const desiredEvidence=new Map();Object.entries(evidence).forEach(([c,arr])=>arr.forEach((e,i)=>desiredEvidence.set(`${c}|${e[0]}`,i+1)));
 (s.groups||[]).forEach(g=>(g.students||[]).forEach(st=>{
  const prefix=`${g.id}|${st.id}|`;
  const scoreByCU={}; const markByCU={};
  Object.entries(oldEvidence).forEach(([c,arr])=>arr.forEach((up,i)=>{const k=`${prefix}${c}|${i+1}`;if(oldScores[k]!==undefined&&oldScores[k]!==null&&oldScores[k]!==''){scoreByCU[`${c}|${up}`]=oldScores[k];if(oldMarks[k])markByCU[`${c}|${up}`]=oldMarks[k]}}));
  // Clear criterion score slots for this student before rebuilding them against the source-of-truth matrix.
  Object.keys(newScores).filter(k=>k.startsWith(prefix)).forEach(k=>delete newScores[k]);
  Object.keys(newMarks).filter(k=>k.startsWith(prefix)).forEach(k=>delete newMarks[k]);
  criteria.forEach(([c])=>{
   (evidence[c]||[]).forEach((e,i)=>{const up=e[0],cu=`${c}|${up}`,qk=`${prefix}${c}|${up}`,sk=`${prefix}${c}|${i+1}`;let v=scoreByCU[cu];let mark=markByCU[cu];if((v===undefined||v===null||v==='')&&oldQual[qk]!==undefined&&oldQual[qk]!==null&&oldQual[qk]!=='')v=oldQual[qk];if(v!==undefined&&v!==null&&v!==''){newScores[sk]=v;if(mark)newMarks[sk]=mark}delete newQual[qk];delete newDates[qk]});
  });
  Object.entries(compMatrix).forEach(([c,arr])=>arr.forEach(up=>{const cu=`${c}|${up}`,qk=`${prefix}${c}|${up}`;if(oldQual[qk]!==undefined&&oldQual[qk]!==null&&oldQual[qk]!==''){newQual[qk]=oldQual[qk];if(oldDates[qk])newDates[qk]=oldDates[qk]}else if(scoreByCU[cu]!==undefined&&scoreByCU[cu]!==null&&scoreByCU[cu]!==''){newQual[qk]=scoreByCU[cu]}}));
  // Remove obsolete qual observations for this student's known matrix criteria.
  Object.keys(newQual).filter(k=>k.startsWith(prefix)).forEach(k=>{const parts=k.split('|'),c=parts[2],up=parts[3];if(criteria.some(x=>x[0]===c)&&!desiredQual.has(`${c}|${up}`)){delete newQual[k];delete newDates[k]}});
 }));
 s.scores=newScores;s.rubricMarks=newMarks;s.qual=newQual;s.qualDates=newDates;
 // Previous CE3 closures may depend on values whose ●/○ role has changed; invalidate them rather than keep stale grades.
 s.continuousClosures={};
 s.matrixSchema=MATRIX_SCHEMA_V3911;
 return s;
}

function migrate(raw){const d=defaultState(),s=Object.assign(d,raw||{});s.attendance=s.attendance||{};s.notes=s.notes||{};s.teacherNotes=s.teacherNotes||{};s.rubricMarks=s.rubricMarks||{};s.fitness=s.fitness||{};s.reflections=s.reflections||{};s.warmups=s.warmups||{};s.up3RPE=s.up3RPE||{};s.up3Tasks=s.up3Tasks||{};s.up4Game=s.up4Game||{};s.photos=s.photos||{};s.qual=s.qual||{};s.qualDates=s.qualDates||{};s.continuousClosures=s.continuousClosures||{};Object.keys(s.qual).forEach(k=>{const v=s.qual[k];if(v==='En proceso')s.qual[k]=4;else if(v==='Adecuado')s.qual[k]=8;else if(v==='Consolidado')s.qual[k]=10});s.selectedDate=s.selectedDate||todayISO();const old=s.evalConfig||{},periods=Object.assign({},defaultUpPeriods(),old.upPeriods||{});if(!old.upPeriods&&(old.firstCut||old.secondCut)){const a=upIndex(old.firstCut||'UP4'),b=upIndex(old.secondCut||'UP10');ups.forEach(([u],i)=>periods[u]=i<=a?'eval1':i<=b?'eval2':'eval3')}s.evalConfig={upPeriods:periods};s.ui=Object.assign({scoreMode:'numeric',evaluationPeriod:'eval1',openWarmup:null,openUp3RPE:null,openUp3Task:null,openUp4Game:null,openSummaryStudent:null,openEvaluateStudent:null},s.ui||{});return migrateToMatrixV3911(s)}
function load(){
 try{
  ...
  if(s.matrixSchema===MATRIX_SCHEMA_V3911){try{localStorage.setItem(APP_KEY,JSON.stringify({...s,photos:{}}))}catch{}}
  return s;
...
function weightedEvidenceValue(vals,c){if(!vals.length)return null;let num=0,den=0;vals.forEach((v,i)=>{const w=i+1;num+=v*w;den+=w});return num/den}
function criterionValue(gid,sid,c){const items=[];(evidence[c]||[]).forEach((e,i)=>{const v=state.scores[scoreKey(gid,sid,c,i+1)];if(v!==undefined&&v!==null&&v!=='')items.push({up:e[0],order:0,v:Number(v)})});Object.entries(state.continuousClosures||{}).forEach(([k,o])=>{const [gg,ss,cc,up]=k.split('|');if(gg===gid&&ss===sid&&cc===c&&o&&o.value!==undefined)items.push({up,order:1,v:Number(o.value)})});items.sort((a,b)=>{const ai=upIndex(a.up),bi=upIndex(b.up);return (ai<0?999:ai)-(bi<0?999:bi)||a.order-b.order});return weightedEvidenceValue(items.map(x=>x.v),c)}
...
function coverage(gid,sid){return Object.entries(ceWeights).reduce((a,[ce,w])=>a+(ceValue(gid,sid,ce)!==null?w:0),0)}
...
function continuousEvaluationHTML(comps){
 ...
 const rows=comps.map(ev=>{const k=qualKey(g.id,sid,ev.c,ev.up),v=state.qual[k],done=v!==undefined&&v!==null&&v!=='';
  return `<div class="quick-eval-row continuous-row ${done?'done':''}">
   <div class="quick-eval-head"><div><span class="pill continuous">○ ${criterionLabel(ev.c)}</span><b class="quick-focus">${esc(quickFocus(ev.up,ev.c,criterionShort[ev.c]||''))}</b></div><span class="quick-status continuous-status">${done?`✓ ${labels[Number(v)]||''} · ${v}`:'Pendiente'}</span></div>
...
function formalCE3ClosureHTML(g,st,period){
 if(!['eval1','eval2','final'].includes(period))return '';
...
function summaryStudentHTML(g,st,period,idx){
 ...
 <div class="summary-ce-grid">${ces.map((v,i)=>{const ce='CE'+(i+1);return `<div><span>${ce} · ${ceWeights[ce]}%${i===2&&period==='eval2'?'<small>continua</small>':''}</span><b>${v===null?'—':v.toFixed(2)}</b></div>`}).join('')}</div>
...
function viewResumen(){
 ...
 return `${groupSelectors()}
 <div class="card evaluation-panel"><div class="evaluation-head"><div><h2>Evaluaciones</h2><p class="muted">Las observaciones ○ de seguimiento de 3.1, 3.2 y 3.3 se acumulan y pueden cerrarse formalmente al final de cada evaluación. Las evidencias ● se mantienen según la matriz. La trayectoria ○ prioriza lo reciente (15% · 25% · 60%); los cierres anteriores no se vuelven a promediar.</p></div></div>
...
function auditData(){const sums={};criteria.forEach(([c,ce])=>{const es=evidence[c]||[],n=es.length;if(!n)return;const nCrit=criteria.filter(x=>x[1]===ce).length,criterionPct=ceWeights[ce]/nCrit,den=n*(n+1)/2;es.forEach((e,i)=>{const share=criterionPct*(i+1)/den;sums[e[1]]=(sums[e[1]]||0)+share})});return sums}
