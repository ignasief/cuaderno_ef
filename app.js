...
function defaultState(){return {groups:[],scores:{},rubricMarks:{},qual:{},attendance:{},notes:{},fitness:{},reflections:{},warmups:{},up3RPE:{},up3Tasks:{},up4Game:{},photos:{},selectedGroup:null,selectedUP:'UP1',selectedDate:todayISO(),evalConfig:{firstCut:'UP4',secondCut:'UP10'},ui:{scoreMode:'numeric',evaluationPeriod:'eval1',openWarmup:null,openUp3RPE:null,openUp3Task:null,openUp4Game:null,openSummaryStudent:null,openEvaluateStudent:null}}}
...
function evalRange(period){if(period==='eval1')return {start:'UP1',end:state.evalConfig.firstCut};if(period==='eval2'){const i=upIndex(state.evalConfig.firstCut);return {start:ups[Math.min(i+1,ups.length-1)][0],end:state.evalConfig.secondCut}}return {start:'UP1',end:null}}
function evalCut(period){return evalRange(period).end}
...
function criterionValueForPeriod(gid,sid,c,period){
 const r=evalRange(period);
 if(period==='final')return criterionValueRange(gid,sid,c,'UP1',null);
 if(period==='eval2'&&isContinuousCriterion(c))return criterionValueRange(gid,sid,c,'UP1',r.end);
 return criterionValueRange(gid,sid,c,r.start,r.end);
}
...
function evidenceInPeriod(c,up,period){const ui=upIndex(up),r=evalRange(period);if(period==='final')return true;if(period==='eval2'&&isContinuousCriterion(c))return ui>=upIndex('UP1')&&ui<=upIndex(r.end);return ui>=upIndex(r.start)&&ui<=upIndex(r.end)}
function evalRangeText(period){const r=evalRange(period);if(period==='final')return 'Acumulativa: todas las evidencias del curso';if(period==='eval2')return `Periodo general: ${r.start} · ${upName(r.start)} → ${r.end} · ${upName(r.end)} · CE3 (3.1, 3.2 y 3.3) continua y acumulada hasta ${r.end}`;return `Periodo: ${r.start} · ${upName(r.start)} → ${r.end} · ${upName(r.end)}`}
...
function quickEvaluationHTML(evs){
 ...
 const rows=evs.map((ev,i)=>{const k=scoreKey(g.id,sid,ev.c,ev.seq),v=state.scores[k],done=v!==undefined&&v!==null&&v!=='',rubric=rubricFor(ev),mark=state.rubricMarks[k],active=i===firstPending,focus=quickFocus(ev.up,ev.c,ev.desc);
  return `<div class="quick-eval-row ${done?'done':''} ${active?'current':''}" id="quick-${esc(ev.c.replace('.','-'))}-${ev.seq}">
   ...
   <div class="quick-rubric-details"><button type="button" class="quick-toggle-btn" data-quick-rubric-toggle="${k}" aria-expanded="false">Ver qué significa cada nivel <span>›</span></button><div class="quick-rubric-levels" data-quick-rubric-panel="${k}" hidden>${rubric.map(r=>`<div><b>${r.score} · ${esc(r.label)}</b><span>${esc(r.desc)}</span></div>`).join('')}</div></div>
...
function viewResumen(){
 ...
 <div class="card evaluation-panel"><div class="evaluation-head"><div><h2>Evaluaciones</h2><p class="muted">La 1.ª y 2.ª evaluación muestran las evidencias de su periodo; CE3 (3.1, 3.2 y 3.3) mantiene seguimiento continuo. La evaluación final integra todo el curso.</p></div></div>
 ...
 <div class="evaluation-cut"><b>${label}</b><span>${esc(evalRangeText(period))}</span></div></div>
...
function viewAjustes(){
 ...
 ${photosHTML}
 <div class="card"><h2>Cortes de evaluación</h2><p class="muted">La 1.ª y 2.ª evaluación se calculan por periodos. Excepción: CE3 (3.1, 3.2 y 3.3) es continua y en la 2.ª evaluación mantiene la trayectoria acumulada desde el inicio de curso. La evaluación final es acumulativa con todas las evidencias del curso. Ajusta aquí qué UP cierra cada periodo.</p><div class="row"><div><label>Fin de 1.ª evaluación</label><select id="firstEvalCut">${ups.map(([u,n])=>`<option value="${u}" ${state.evalConfig.firstCut===u?'selected':''}>${u} · ${esc(n)}</option>`).join('')}</select></div><div><label>Fin de 2.ª evaluación</label><select id="secondEvalCut">${ups.map(([u,n])=>`<option value="${u}" ${state.evalConfig.secondCut===u?'selected':''}>${u} · ${esc(n)}</option>`).join('')}</select></div></div><p class="tiny">Configuración inicial: 1.ª evaluación hasta UP4 y 2.ª hasta UP10. Cámbiala si tu calendario definitivo sitúa los cierres en otras unidades.</p></div>
...
function bind(){document.querySelectorAll('[data-go]').forEach(b=>b.onclick=()=>{currentView=b.dataset.go;render()});document.querySelectorAll('.bottom-nav button').forEach(b=>b.onclick=()=>{currentView=b.dataset.view;render()});
 ...
 document.querySelectorAll('[data-quick-rubric-toggle]').forEach(b=>b.onclick=(e)=>{e.preventDefault();e.stopPropagation();const k=b.dataset.quickRubricToggle,p=document.querySelector(`[data-quick-rubric-panel="${CSS.escape(k)}"]`);if(!p)return;const opening=p.hidden;p.hidden=!opening;b.setAttribute('aria-expanded',opening?'true':'false');b.classList.toggle('open',opening)});
 document.querySelectorAll('[data-eval-tools-toggle]').forEach(b=>b.onclick=(e)=>{e.preventDefault();e.stopPropagation();const p=b.parentElement?.querySelector('[data-eval-tools-panel]');if(!p)return;const opening=p.hidden;p.hidden=!opening;b.setAttribute('aria-expanded',opening?'true':'false');b.classList.toggle('open',opening)});
...
function exportCSV(){const g=group();if(!g){alert('Selecciona un grupo');return}const period=state.ui.evaluationPeriod||'eval1',range=evalRange(period),head=['Alumno','Evaluación','Peso evaluado','Criterios evaluados','CE1','CE2','CE3','CE4','CE5','Nota evaluación','Nivel criterial acumulado'];const rows=g.students.map(st=>[st.name,evalLabel(period),coverageRange(g.id,st.id,range.start,range.end),assessedCountForPeriod(g.id,st.id,period),...['CE1','CE2','CE3','CE4','CE5'].map(ce=>ceValueForPeriod(g.id,st.id,ce,period)?.toFixed(2)||''),gradeForPeriod(g.id,st.id,period)?.toFixed(2)||'',finalGrade(g.id,st.id)?.toFixed(2)||'']);download(`resumen-${period}-${g.name}.csv`,[head,...rows].map(r=>r.map(x=>'"'+String(x).replaceAll('"','""')+'"').join(';')).join('\n'),'text/csv;charset=utf-8')}
