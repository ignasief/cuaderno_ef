...
function defaultState(){return {groups:[],scores:{},rubricMarks:{},qual:{},attendance:{},notes:{},fitness:{},reflections:{},warmups:{},up3RPE:{},up3Tasks:{},photos:{},selectedGroup:null,selectedUP:'UP1',selectedDate:todayISO(),evalConfig:{firstCut:'UP4',secondCut:'UP10'},ui:{scoreMode:'numeric',evaluationPeriod:'eval1',openWarmup:null,openUp3RPE:null,openUp3Task:null}}}
function migrate(raw){const d=defaultState(),s=Object.assign(d,raw||{});s.attendance=s.attendance||{};s.notes=s.notes||{};s.rubricMarks=s.rubricMarks||{};s.fitness=s.fitness||{};s.reflections=s.reflections||{};s.warmups=s.warmups||{};s.up3RPE=s.up3RPE||{};s.up3Tasks=s.up3Tasks||{};s.photos=s.photos||{};s.selectedDate=s.selectedDate||todayISO();s.evalConfig=Object.assign({firstCut:'UP4',secondCut:'UP10'},s.evalConfig||{});s.ui=Object.assign({scoreMode:'numeric',evaluationPeriod:'eval1',openWarmup:null,openUp3RPE:null,openUp3Task:null},s.ui||{});return s}
...
<div class="card"><h3>Seguimiento complementario ○</h3>${comps.length?comps.map((e,i)=>`<div class="evidence-card"><div><span class="pill comp">○ ${criterionLabel(e.c)}</span> Seguimiento cualitativo</div><button class="btn small secondary" data-open-comp="${i}">Registrar</button></div>`).join(''):`<div class="empty">No hay evidencias ○ previstas en esta UP.</div>`}</div><div id="sessionOverlay"></div>`}
...
function up3TaskInstrumentHTML(){
 ...
 <div class="up3-task-suggestions"><div class="warmup-suggestion"><div><span>Sugerencia 1.1</span><strong>${s11?`${s11.label} · ${s11.score}`:'Completa sus 2 indicadores'}</strong><small>Juicio docente final.</small></div>${s11?`<button class="btn small" data-apply-up3-task="${k}" data-up3-task-sid="${st.id}" data-up3-task-criterion="1.1" data-up3-task-score="${s11.score}" data-up3-task-level="${s11.label}">${r11?'Actualizar':'Registrar'} ${s11.score} en 1.1</button>`:''}</div>
 <div class="warmup-suggestion"><div><span>Sugerencia 1.2</span><strong>${s12?`${s12.label} · ${s12.score}`:'Completa sus 2 indicadores'}</strong><small>Juicio docente final.</small></div>${s12?`<button class="btn small" data-apply-up3-task="${k}" data-up3-task-sid="${st.id}" data-up3-task-criterion="1.2" data-up3-task-score="${s12.score}" data-up3-task-level="${s12.label}">${r12?'Actualizar':'Registrar'} ${s12.score} en 1.2</button>`:''}</div></div>
...
function viewResumen(){
 ...
 <div class="card"><h2>${label}</h2><div class="desktop-summary table-wrap"><table><thead><tr><th>Alumno/a</th><th>Cobertura</th><th>Criterios</th><th>CE1</th><th>CE2</th><th>CE3</th><th>CE4</th><th>CE5</th><th>Nota evaluación</th></tr></thead><tbody>${rows.map(r=>`<tr><td class="name"><div class="student-with-avatar">${avatarHTML(g.id,r.s,'sm')}<div class="student-name-text"><b>${esc(r.s.name)}</b></div></div></td><td>${r.cov.toFixed(0)}%</td><td>${r.count}/19</td>${r.ces.map(v=>`<td>${v===null?'—':v.toFixed(2)}</td>`).join('')}<td class="${r.grade===null?'':r.grade<5?'bad':r.grade<7?'warn':'ok'}"><b>${r.grade===null?'—':r.grade.toFixed(2)}</b></td></tr>`).join('')}</tbody></table></div>
 ...
 <div class="card current-level-card"><h3>Nivel criterial actual</h3><p class="muted">Indicador de progreso acumulado con todas las evidencias registradas hasta hoy. No es, por sí solo, una nota trimestral.</p><div class="current-level-grid">${rows.map(r=>`<div><span class="student-with-avatar">${avatarHTML(g.id,r.s,'sm')}<span>${esc(r.s.name)}</span></span><b>${r.current===null?'—':r.current.toFixed(2)}</b></div>`).join('')}</div></div>
...
function bind(){document.querySelectorAll('[data-go]').forEach(b=>b.onclick=()=>{currentView=b.dataset.go;render()});document.querySelectorAll('.bottom-nav button').forEach(b=>b.onclick=()=>{currentView=b.dataset.view;render()});
 ...
 document.querySelectorAll('[data-select-group]').forEach(b=>b.onclick=()=>{state.selectedGroup=b.dataset.selectGroup;save();render()});document.querySelectorAll('[data-delete-group]').forEach(b=>b.onclick=()=>{if(confirm('¿Borrar este grupo y sus datos?')){const gid=b.dataset.deleteGroup;state.groups=state.groups.filter(g=>g.id!==gid);[state.scores,state.rubricMarks,state.qual,state.attendance,state.notes,state.fitness,state.reflections,state.warmups,state.up3RPE,state.up3Tasks,state.photos].forEach(obj=>Object.keys(obj).filter(k=>k.startsWith(gid+'|')).forEach(k=>delete obj[k]));state.selectedGroup=state.groups[0]?.id||null;save();render()}});
