...
function defaultState(){return {groups:[],scores:{},rubricMarks:{},qual:{},attendance:{},notes:{},fitness:{},reflections:{},selectedGroup:null,selectedUP:'UP1',selectedDate:todayISO(),ui:{scoreMode:'numeric'}}}
function migrate(raw){const d=defaultState(),s=Object.assign(d,raw||{});s.attendance=s.attendance||{};s.notes=s.notes||{};s.rubricMarks=s.rubricMarks||{};s.fitness=s.fitness||{};s.reflections=s.reflections||{};s.selectedDate=s.selectedDate||todayISO();s.ui=Object.assign({scoreMode:'numeric'},s.ui||{});return s}
...
function bind(){document.querySelectorAll('[data-go]').forEach(b=>b.onclick=()=>{currentView=b.dataset.go;render()});document.querySelectorAll('.bottom-nav button').forEach(b=>b.onclick=()=>{currentView=b.dataset.view;render()});
 ...
 document.querySelectorAll('[data-select-group]').forEach(b=>b.onclick=()=>{state.selectedGroup=b.dataset.selectGroup;save();render()});document.querySelectorAll('[data-delete-group]').forEach(b=>b.onclick=()=>{if(confirm('¿Borrar este grupo y sus datos?')){const gid=b.dataset.deleteGroup;state.groups=state.groups.filter(g=>g.id!==gid);[state.scores,state.rubricMarks,state.qual,state.attendance,state.notes,state.fitness,state.reflections].forEach(obj=>Object.keys(obj).filter(k=>k.startsWith(gid+'|')).forEach(k=>delete obj[k]));state.selectedGroup=state.groups[0]?.id||null;save();render()}});
