...
function criterionValueForPeriod(gid,sid,c,period){const vals=[];(evidence[c]||[]).forEach((e,i)=>{if(!evidenceInPeriod(c,e[0],period))return;const v=state.scores[scoreKey(gid,sid,c,i+1)];if(v!==undefined&&v!==null&&v!=='')vals.push(Number(v))});if(!vals.length)return null;let num=0,den=0;vals.forEach((v,i)=>{const w=i+1;num+=v*w;den+=w});return num/den}
