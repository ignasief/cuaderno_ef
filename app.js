...
function defaultState(){return {groups:[],scores:{},rubricMarks:{},qual:{},attendance:{},notes:{},fitness:{},reflections:{},warmups:{},selectedGroup:null,selectedUP:'UP1',selectedDate:todayISO(),ui:{scoreMode:'numeric'}}}
function migrate(raw){const d=defaultState(),s=Object.assign(d,raw||{});s.attendance=s.attendance||{};s.notes=s.notes||{};s.rubricMarks=s.rubricMarks||{};s.fitness=s.fitness||{};s.reflections=s.reflections||{};s.warmups=s.warmups||{};s.selectedDate=s.selectedDate||todayISO();s.ui=Object.assign({scoreMode:'numeric'},s.ui||{});return s}
...
function viewResumen(){const g=group();if(!g)return `<div class="card empty">Primero crea un grupo en Ajustes.</div>`;const rows=g.students.map(s=>{const cov=coverage(g.id,s.id),grade=finalGrade(g.id,s.id),ces=['CE1','CE2','CE3','CE4','CE5'].map(ce=>ceValue(g.id,s.id,ce));return {s,cov,grade,ces}});return `${groupSelectors()}<div class="card"><h2>Resumen del grupo</h2><div class="desktop-summary table-wrap"><table><thead><tr><th>Alumno/a</th><th>Cobertura</th><th>CE1</th><th>CE2</th><th>CE3</th><th>CE4</th><th>CE5</th><th>Nota</th></tr></thead><tbody>${rows.map(r=>`<tr><td class="name">${esc(r.s.name)}</td><td>${r.cov.toFixed(0)}%</td>${r.ces.map(v=>`<td>${v===null?'—':v.toFixed(2)}</td>`).join('')}<td class="${r.grade===null?'':r.grade<5?'bad':r.grade<7?'warn':'ok'}"><b>${r.grade===null?'—':r.grade.toFixed(2)}</b></td></tr>`).join('')}</tbody></table></div><div class="mobile-summary">${rows.map(r=>`<div class="summary-student"><div class="summary-top"><b>${esc(r.s.name)}</b><span class="grade-chip ${r.grade===null?'':r.grade<5?'bad':r.grade<7?'warn':'ok'}">${r.grade===null?'—':r.grade.toFixed(2)}</span></div><div class="summary-meta"><span>Cobertura <b>${r.cov.toFixed(0)}%</b></span>${r.ces.map((v,i)=>`<span>CE${i+1} <b>${v===null?'—':v.toFixed(2)}</b></span>`).join('')}</div></div>`).join('')}</div></div>
<div class="card"><h3>Detalle individual</h3><label>Alumno/a</label><select id="studentDetailSel">${g.students.map(s=>`<option value="${s.id}">${esc(s.name)}</option>`).join('')}</select><div id="studentDetail"></div></div>`}
function detailHTML(sid){const g=group(),s=g.students.find(x=>x.id===sid);if(!s)return '';const history=[];criteria.forEach(([c])=>(evidence[c]||[]).forEach((e,i)=>{const v=state.scores[scoreKey(g.id,s.id,c,i+1)];if(v!==undefined&&v!==null&&v!==''){const k=scoreKey(g.id,s.id,c,i+1),mark=state.rubricMarks[k];history.push({c,up:e[0],desc:e[2],v,seq:i+1,mark})}}));return `<div class="table-wrap" style="margin-top:10px"><table><thead><tr><th>Criterio</th><th>Peso</th><th>Nivel actual</th></tr></thead><tbody>${criteria.map(([c,,w])=>{const v=criterionValue(g.id,s.id,c);return `<tr><td>${c}</td><td>${w}%</td><td>${v===null?'—':v.toFixed(2)}</td></tr>`}).join('')}</tbody></table></div><h3 style="margin-top:16px">Historial de evidencias ●</h3>${history.length?history.map(h=>`<div class="history-item"><b>${h.c} · ${h.up} · ${h.mark?esc(h.mark.label)+' · ':''}${h.v}</b><br>${esc(h.desc)}${h.mark?`<div class="history-rubric">${esc(h.mark.desc)}</div>`:''}</div>`).join(''):`<div class="muted">Todavía no hay evidencias principales registradas.</div>`}`}
...
function exportCSV(){const g=group();if(!g){alert('Selecciona un grupo');return}const head=['Alumno','Cobertura','CE1','CE2','CE3','CE4','CE5','Nota'];const rows=g.students.map(s=>[s.name,coverage(g.id,s.id),...['CE1','CE2','CE3','CE4','CE5'].map(ce=>ceValue(g.id,s.id,ce)?.toFixed(2)||''),finalGrade(g.id,s.id)?.toFixed(2)||'']);download(`resumen-${g.name}.csv`,[head,...rows].map(r=>r.map(x=>'"'+String(x).replaceAll('"','""')+'"').join(';')).join('\n'),'text/csv;charset=utf-8')}
