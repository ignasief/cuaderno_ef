...
function viewAjustes(){
 ...
 const groupsHTML=state.groups.map(g=>`<div class="card group-admin" style="margin-top:10px"><div class="row"><div><b>${esc(g.name)}</b><div class="tiny">${g.students.length} alumnos</div></div><button class="btn small secondary" data-select-group="${g.id}">Usar</button><button class="btn small danger" data-delete-group="${g.id}">Borrar</button></div><div class="row" style="margin-top:8px"><textarea id="bulk-${g.id}" rows="3" placeholder="Pega un alumno por línea"></textarea><button class="btn small" data-add-students="${g.id}">Añadir</button></div></div>`).join('');
 const photosHTML=active?`<div class="card"><h2>Alumnado y fotos</h2><p class="muted">Grupo activo: <b>${esc(active.name)}</b>. Las fotos se guardan solo en este dispositivo y se incluyen en la copia JSON.</p>${active.students.length?`<div class="student-photo-list always-visible">${active.students.map(st=>{const has=!!state.photos[photoKey(active.id,st.id)];return `<div class="student-photo-row">${avatarHTML(active.id,st,'lg')}<div class="student-photo-name"><b>${esc(st.name)}</b><span>${has?'Foto guardada':'Sin foto'}</span></div><div class="photo-actions"><label class="btn small secondary photo-upload-btn">${has?'Cambiar':'Elegir foto'}<input type="file" accept="image/*" data-photo-upload="${active.id}|${st.id}" hidden></label><label class="btn small secondary photo-upload-btn camera-btn">Cámara<input type="file" accept="image/*" capture="user" data-photo-upload="${active.id}|${st.id}" hidden></label>${has?`<button class="btn small danger" data-remove-photo="${active.id}|${st.id}">Quitar</button>`:''}</div></div>`}).join('')}</div>`:`<div class="muted">Añade alumnado al grupo para poder asignar fotografías.</div>`}</div>`:'';
