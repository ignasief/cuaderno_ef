...
function quickEvaluationHTML(evs){
 ...
 const rows=evs.map((ev,i)=>{const k=scoreKey(g.id,sid,ev.c,ev.seq),v=state.scores[k],done=v!==undefined&&v!==null&&v!=='',rubric=rubricFor(ev),mark=state.rubricMarks[k],active=i===firstPending,focus=quickFocus(ev.up,ev.c,ev.desc);
  return `<div class="quick-eval-row ${done?'done':''} ${active?'current':''}" id="quick-${esc(ev.c.replace('.','-'))}-${ev.seq}">
   ...
   <div class="quick-rubric-details css-toggle-wrap"><input class="css-toggle-input" type="checkbox" id="rubric-toggle-${i}"><label class="quick-toggle-btn" for="rubric-toggle-${i}">Ver qué significa cada nivel <span>›</span></label><div class="quick-rubric-levels css-toggle-panel">${rubric.map(r=>`<div><b>${r.score} · ${esc(r.label)}</b><span>${esc(r.desc)}</span></div>`).join('')}</div></div>
...
function evaluateStudentBody(evs,comps){
 ...
 <div class="eval-detail-tools css-toggle-wrap"><input class="css-toggle-input" type="checkbox" id="eval-tools-toggle"><label class="eval-detail-toggle" for="eval-tools-toggle">Herramientas detalladas y observaciones <span>›</span></label><div class="eval-detail-body css-toggle-panel">${fitnessPassportHTML()}${up1ReflectionHTML()}${up2WarmupInstrumentHTML()}${up3RPEHTML()}${up3TaskInstrumentHTML()}${up4GameInstrumentHTML()}
