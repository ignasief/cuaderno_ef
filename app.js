...
function defaultState(){return {groups:[],scores:{},rubricMarks:{},qual:{},attendance:{},notes:{},fitness:{},reflections:{},warmups:{},photos:{},selectedGroup:null,selectedUP:'UP1',selectedDate:todayISO(),evalConfig:{firstCut:'UP4',secondCut:'UP10'},ui:{scoreMode:'numeric',evaluationPeriod:'eval1',openWarmup:null}}}
function migrate(raw){const d=defaultState(),s=Object.assign(d,raw||{});s.attendance=s.attendance||{};s.notes=s.notes||{};s.rubricMarks=s.rubricMarks||{};s.fitness=s.fitness||{};s.reflections=s.reflections||{};s.warmups=s.warmups||{};s.photos=s.photos||{};s.selectedDate=s.selectedDate||todayISO();s.evalConfig=Object.assign({firstCut:'UP4',secondCut:'UP10'},s.evalConfig||{});s.ui=Object.assign({scoreMode:'numeric',evaluationPeriod:'eval1',openWarmup:null},s.ui||{});return s}
...
function qualListHTML(c,up){const g=group();return g.students.map(s=>{const k=qualKey(g.id,s.id,c,up),v=state.qual[k]||'';return `<div class="student-row"><div class="student-name student-with-avatar">${avatarHTML(g.id,s,'sm')}<div class="student-name-text"><b>${esc(s.name)}</b><div class="tiny">${c} · ${up}</div></div></div><select data-qual-key="${k}"><option value="">—</option>${['En proceso','Adecuado','Consolidado'].map(x=>`<option ${v===x?'selected':''}>${x}</option>`).join('')}</select></div>`}).join('')}
function viewResumen(){
 ...
 <div class="mobile-summary">${rows.map(r=>`<div class="summary-student"><div class="summary-top"><div class="student-with-avatar">${avatarHTML(g.id,r.s,'sm')}<div class="student-name-text"><b>${esc(r.s.name)}</b><small>Nivel criterial actual: ${r.current===null?'—':r.current.toFixed(2)}</small></div></div><span class="grade-chip ${r.grade===null?'':r.grade<5?'bad':r.grade<7?'warn':'ok'}">${r.grade===null?'—':r.grade.toFixed(2)}</span></div><div class="summary-meta"><span>Cobertura <b>${r.cov.toFixed(0)}%</b></span><span>Criterios <b>${r.count}/19</b></span>${r.ces.map((v,i)=>`<span>CE${i+1} <b>${v===null?'—':v.toFixed(2)}</b></span>`).join('')}</div></div>`).join('')}</div></div>
...
function viewAjustes(){const a=auditData();return `<div class="card"><h2>Grupos y alumnado</h2><div class="row"><input id="newGroup" placeholder="Ej. 1.º ESO A"><button class="btn" id="addGroup">Crear grupo</button></div>${state.groups.map(g=>`<div class="card group-admin" style="margin-top:10px"><div class="row"><div><b>${esc(g.name)}</b><div class="tiny">${g.students.length} alumnos</div></div><button class="btn small secondary" data-select-group="${g.id}">Usar</button><button class="btn small danger" data-delete-group="${g.id}">Borrar</button></div>${g.students.length?`<div class="student-photo-list">${g.students.map(st=>`<div class="student-photo-row">${avatarHTML(g.id,st,'lg')}<div class="student-photo-name"><b>${esc(st.name)}</b><span>${state.photos[photoKey(g.id,st.id)]?'Foto guardada en este dispositivo':'Sin foto'}</span></div><label class="btn small secondary photo-upload-btn">${state.photos[photoKey(g.id,st.id)]?'Cambiar foto':'Añadir foto'}<input type="file" accept="image/*" data-photo-upload="${g.id}|${st.id}" hidden></label>${state.photos[photoKey(g.id,st.id)]?`<button class="btn small danger" data-remove-photo="${g.id}|${st.id}">Quitar</button>`:''}</div>`).join('')}</div>`:''}<div class="row" style="margin-top:8px"><textarea id="bulk-${g.id}" rows="3" placeholder="Pega un alumno por línea"></textarea><button class="btn small" data-add-students="${g.id}">Añadir</button></div></div>`).join('')}</div>
<div class="card"><h2>Cortes de evaluación</h2><p class="muted">Las evaluaciones son acumulativas. Puedes ajustar qué UP cierra cada trimestre según tu temporalización real.</p><div class="row"><div><label>Fin de 1.ª evaluación</label><select id="firstEvalCut">${ups.map(([u,n])=>`<option value="${u}" ${state.evalConfig.firstCut===u?'selected':''}>${u} · ${esc(n)}</option>`).join('')}</select></div><div><label>Fin de 2.ª evaluación</label><select id="secondEvalCut">${ups.map(([u,n])=>`<option value="${u}" ${state.evalConfig.secondCut===u?'selected':''}>${u} · ${esc(n)}</option>`).join('')}</select></div></div><p class="tiny">Configuración inicial: 1.ª evaluación hasta UP4 y 2.ª hasta UP10. Cámbiala si tu calendario definitivo sitúa los cierres en otras unidades.</p></div>
...
function bind(){document.querySelectorAll('[data-go]').forEach(b=>b.onclick=()=>{currentView=b.dataset.go;render()});document.querySelectorAll('.bottom-nav button').forEach(b=>b.onclick=()=>{currentView=b.dataset.view;render()});
 ...
 document.querySelectorAll('[data-remove-photo]').forEach(b=>b.onclick=()=>{if(confirm('¿Quitar la foto de este alumno?')){delete state.photos[b.dataset.removePhoto];save();render()}});
