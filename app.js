...
function makeRubric(proc,focus){
 ...
 return ['Inicio','Básico','Adecuado','Avanzado'].map((label,i)=>({label,score:[4,6,8,10][i],desc:ds[i]}));
}
...
function defaultState(){return {groups:[],scores:{},rubricMarks:{},qual:{},attendance:{},notes:{},fitness:{},reflections:{},warmups:{},selectedGroup:null,selectedUP:'UP1',selectedDate:todayISO(),evalConfig:{firstCut:'UP4',secondCut:'UP8'},ui:{scoreMode:'numeric',evaluationPeriod:'eval1'}}}
function migrate(raw){const d=defaultState(),s=Object.assign(d,raw||{});s.attendance=s.attendance||{};s.notes=s.notes||{};s.rubricMarks=s.rubricMarks||{};s.fitness=s.fitness||{};s.reflections=s.reflections||{};s.warmups=s.warmups||{};s.selectedDate=s.selectedDate||todayISO();s.evalConfig=Object.assign({firstCut:'UP4',secondCut:'UP8'},s.evalConfig||{});s.ui=Object.assign({scoreMode:'numeric',evaluationPeriod:'eval1'},s.ui||{});return s}
...
function group(){return state.groups.find(g=>g.id===state.selectedGroup)||state.groups[0]||null}
...
function criterionValue(gid,sid,c){const es=evidence[c]||[];let num=0,den=0;es.forEach((_,i)=>{const v=state.scores[scoreKey(gid,sid,c,i+1)];if(v!==undefined&&v!==null&&v!==''){num+=Number(v)*(i+1);den+=(i+1)}});return den?num/den:null}
...
function criterionValueAt(gid,sid,c,cutUp){const cut=cutUp?upIndex(cutUp):Infinity,es=evidence[c]||[];let num=0,den=0;es.forEach((e,i)=>{if(upIndex(e[0])>cut)return;const v=state.scores[scoreKey(gid,sid,c,i+1)];if(v!==undefined&&v!==null&&v!==''){num+=Number(v)*(i+1);den+=(i+1)}});return den?num/den:null}
function ceValueAt(gid,sid,ce,cutUp){let num=0,den=0;criteria.filter(x=>x[1]===ce).forEach(([c,,w])=>{const v=criterionValueAt(gid,sid,c,cutUp);if(v!==null){num+=v*w;den+=w}});return den?num/den:null}
function coverageAt(gid,sid,cutUp){return criteria.reduce((a,[c,,w])=>a+(criterionValueAt(gid,sid,c,cutUp)!==null?w:0),0)}
function gradeAt(gid,sid,cutUp){let num=0,den=0;criteria.forEach(([c,,w])=>{const v=criterionValueAt(gid,sid,c,cutUp);if(v!==null){num+=v*w;den+=w}});return den?num/den:null}
function assessedCountAt(gid,sid,cutUp){return criteria.filter(([c])=>criterionValueAt(gid,sid,c,cutUp)!==null).length}
function evalCut(period){if(period==='eval1')return state.evalConfig.firstCut;if(period==='eval2')return state.evalConfig.secondCut;return null}
function evalLabel(period){return period==='eval1'?'1.ª evaluación':period==='eval2'?'2.ª evaluación':'Evaluación final'}
...
<div class="card"><h3>Seguimiento complementario ○</h3>${comps.length?comps.map((e,i)=>`<div class="evidence-card"><div><span class="pill comp">○ ${e.c}</span> Seguimiento cualitativo</div><button class="btn small secondary" data-open-comp="${i}">Registrar</button></div>`).join(''):`<div class="empty">No hay evidencias ○ previstas en esta UP.</div>`}</div><div id="sessionOverlay"></div>`}
function attendanceHTML(g){return g.students.map((s,idx)=>{const k=attendanceKey(g.id,s.id,state.selectedDate),v=state.attendance[k]||'';const note=state.notes[noteKey(g.id,s.id,state.selectedDate)]||'';return `<div class="student-row attendance-row"><div class="student-name"><span class="student-index">${idx+1}</span><b>${esc(s.name)}</b>${note?`<div class="tiny note-preview">📝 ${esc(note).slice(0,54)}${note.length>54?'…':''}</div>`:''}</div><div class="attendance-actions"><div class="attendance">${[['P','present'],['A','absent'],['J','justified']].map(([x,cl])=>`<button class="${cl} ${v===x?'active':''}" data-att="${x}" data-att-key="${k}" aria-label="${x==='P'?'Presente':x==='A'?'Ausente':'Ausencia justificada'} ${esc(s.name)}">${x}</button>`).join('')}</div><button class="note-btn" data-note-student="${s.id}" title="Observación">📝</button></div></div>`}).join('')}
...
function fitnessPassportHTML(){
 ...
 ${g.students.map((st,idx)=>{const k=fitnessKey(g.id,st.id),v=state.fitness[k]||{};return `<details class="fitness-student" ${idx===0?'open':''}><summary><b>${esc(st.name)}</b><span>${Object.values(v).filter(x=>x!==''&&x!=null).length}/7 datos</span></summary><div class="fitness-grid">
...
function up1ReflectionHTML(){
 ...
 ${g.students.map((st,idx)=>{const k=reflectionKey(g.id,st.id),v=state.reflections[k]||{};const fields=['effort','strength','improve','evidence','decision','autonomy'];const done=fields.filter(f=>v[f]!==undefined&&String(v[f]).trim()!=='').length;return `<details class="reflection-student" ${idx===0?'open':''}><summary><b>${esc(st.name)}</b><span class="reflection-count">${done}/6 respuestas</span></summary><div class="reflection-grid">
...
function up2WarmupInstrumentHTML(){
 ...
 return `<div class="card warmup-card"><div class="fitness-title"><div><h2>Instrumento · Diseño y dirección del calentamiento</h2><p class="muted">Registro analítico para observar la actuación. Los seis indicadores <b>no son seis notas</b>: generan una sugerencia de nivel para la única evidencia principal del criterio 1.2. La decisión final sigue siendo docente.</p></div><span class="pill main">UP2 · 1.2</span></div>
 ${g.students.map((st,idx)=>{const k=warmupKey(g.id,st.id),v=state.warmups[k]||{};const vals=indicators.map(([f])=>Number(v[f])||0).filter(Boolean);const done=vals.length;const avg=done===indicators.length?vals.reduce((a,b)=>a+b,0)/vals.length:null;const suggested=avg===null?null:(avg<1.75?4:avg<2.75?6:avg<3.75?8:10);const level=suggested===4?'Inicio':suggested===6?'Básico':suggested===8?'Adecuado':suggested===10?'Avanzado':'';return `<details class="warmup-student" ${idx===0?'open':''}><summary><b>${esc(st.name)}</b><span class="warmup-count">${done}/6 indicadores</span></summary><div class="warmup-body">
 ...
 <div class="warmup-suggestion">${suggested?`<div><span>Sugerencia del instrumento</span><strong>${level} · ${suggested}</strong><small>Calculada a partir de los 6 indicadores. Puedes mantenerla o modificar la valoración final en la rúbrica.</small></div><button class="btn small" data-apply-warmup="${k}" data-warm-score="${suggested}" data-warm-level="${level}">Registrar ${suggested} en 1.2</button>`:`<div><span>Sugerencia del instrumento</span><strong>Completa los 6 indicadores</strong><small>No se propone nivel hasta completar la observación.</small></div>`}</div>
 </div></details>`}).join('')}</div>`;
}

function viewEvaluar(){const g=group();if(!g)return `<div class="card empty">Primero crea un grupo en Ajustes.</div>`;const evs=principalForUP(state.selectedUP),comps=compsForUP(state.selectedUP);return `${groupSelectors()}${fitnessPassportHTML()}${up1ReflectionHTML()}${up2WarmupInstrumentHTML()}<div class="card"><div class="row"><div><h2>Evidencias principales ●</h2></div><div class="score-mode"><button data-mode="numeric" class="${state.ui.scoreMode==='numeric'?'active':''}">Numérico</button><button data-mode="levels" class="${state.ui.scoreMode==='levels'?'active':''}">4 niveles</button></div></div>${evs.length?`<label>Evidencia</label><select id="evSel">${evs.map((e,i)=>`<option value="${i}">${e.c} · ${esc(e.desc)}</option>`).join('')}</select><div id="evMeta" class="sticky-evidence"></div><div id="studentScores"></div>`:`<div class="empty">No hay evidencias ● en esta UP.</div>`}</div><div class="card"><h2>Seguimiento complementario ○</h2><p class="muted">Registro formativo: orienta el feedback y la consistencia del aprendizaje, pero no modifica automáticamente la calificación.</p>${comps.length?comps.map(e=>`<div class="comp-block"><div class="comp-head"><span class="pill comp">○ ${e.c}</span> <b>Seguimiento cualitativo</b></div>${qualListHTML(e.c,e.up)}</div>`).join(''):`<div class="empty">No hay evidencias ○ previstas en esta UP.</div>`}</div>`}
function scoreListHTML(ev){const g=group();if(!g)return '';const mode=state.ui.scoreMode,rubric=rubricFor(ev);return g.students.map(s=>{const k=scoreKey(g.id,s.id,ev.c,ev.seq),v=state.scores[k],mark=state.rubricMarks[k];let controls='';if(mode==='levels'){controls=`<div class="level-buttons rubric-buttons">${rubric.map(x=>`<button data-score="${x.score}" data-key="${k}" data-level="${x.label}" data-desc="${esc(x.desc)}" class="${mark?.label===x.label||(!mark&&String(v)===String(x.score))?'selected':''}">${x.label}<small>${x.score}</small></button>`).join('')}<button class="clear-level" data-clear="${k}">×</button></div>${mark?`<div class="student-rubric-desc"><b>${esc(mark.label)} · ${mark.score}</b> — ${esc(mark.desc)}</div>`:''}`}else{controls=`<div class="score-buttons">${[0,4,5,6,7,8,9,10].map(x=>`<button data-score="${x}" data-key="${k}" class="${String(v)===String(x)?'selected':''}">${x}</button>`).join('')}<button data-clear="${k}">×</button></div>`}return `<div class="student-row"><div class="student-name"><b>${esc(s.name)}</b><div class="tiny">Criterio ${ev.c}${v!==undefined&&v!==''?` · actual: ${v}`:''}${mark?` · ${esc(mark.label)}`:''}</div></div>${controls}</div>`}).join('')}
function qualListHTML(c,up){const g=group();return g.students.map(s=>{const k=qualKey(g.id,s.id,c,up),v=state.qual[k]||'';return `<div class="student-row"><div class="student-name"><b>${esc(s.name)}</b><div class="tiny">${c} · ${up}</div></div><select data-qual-key="${k}"><option value="">—</option>${['En proceso','Adecuado','Consolidado'].map(x=>`<option ${v===x?'selected':''}>${x}</option>`).join('')}</select></div>`}).join('')}
function viewResumen(){
 const g=group();if(!g)return `<div class="card empty">Primero crea un grupo en Ajustes.</div>`;
 const period=state.ui.evaluationPeriod||'eval1',cut=evalCut(period),label=evalLabel(period);
 const rows=g.students.map(st=>{const cov=coverageAt(g.id,st.id,cut),grade=gradeAt(g.id,st.id,cut),ces=['CE1','CE2','CE3','CE4','CE5'].map(ce=>ceValueAt(g.id,st.id,ce,cut)),count=assessedCountAt(g.id,st.id,cut),current=finalGrade(g.id,st.id);return {s:st,cov,grade,ces,count,current}});
 return `${groupSelectors()}
 <div class="card evaluation-panel"><div class="evaluation-head"><div><h2>Evaluaciones</h2><p class="muted">La nota se calcula con los criterios realmente evaluados hasta el cierre seleccionado y normaliza sus pesos. Los criterios aún no trabajados no cuentan como cero.</p></div></div>
 ...
 <div class="evaluation-cut"><b>${label}</b>${cut?`<span>Cierre configurado: ${cut} · ${esc(upName(cut))}</span>`:`<span>Acumulativa: todas las evidencias registradas</span>`}</div></div>
 <div class="card"><h2>${label}</h2><div class="desktop-summary table-wrap"><table><thead><tr><th>Alumno/a</th><th>Cobertura</th><th>Criterios</th><th>CE1</th><th>CE2</th><th>CE3</th><th>CE4</th><th>CE5</th><th>Nota evaluación</th></tr></thead><tbody>${rows.map(r=>`<tr><td class="name">${esc(r.s.name)}</td><td>${r.cov.toFixed(0)}%</td><td>${r.count}/19</td>${r.ces.map(v=>`<td>${v===null?'—':v.toFixed(2)}</td>`).join('')}<td class="${r.grade===null?'':r.grade<5?'bad':r.grade<7?'warn':'ok'}"><b>${r.grade===null?'—':r.grade.toFixed(2)}</b></td></tr>`).join('')}</tbody></table></div>
 <div class="mobile-summary">${rows.map(r=>`<div class="summary-student"><div class="summary-top"><div><b>${esc(r.s.name)}</b><small>Nivel criterial actual: ${r.current===null?'—':r.current.toFixed(2)}</small></div><span class="grade-chip ${r.grade===null?'':r.grade<5?'bad':r.grade<7?'warn':'ok'}">${r.grade===null?'—':r.grade.toFixed(2)}</span></div><div class="summary-meta"><span>Cobertura <b>${r.cov.toFixed(0)}%</b></span><span>Criterios <b>${r.count}/19</b></span>${r.ces.map((v,i)=>`<span>CE${i+1} <b>${v===null?'—':v.toFixed(2)}</b></span>`).join('')}</div></div>`).join('')}</div></div>
 <div class="card current-level-card"><h3>Nivel criterial actual</h3><p class="muted">Indicador de progreso acumulado con todas las evidencias registradas hasta hoy. No es, por sí solo, una nota trimestral.</p><div class="current-level-grid">${rows.map(r=>`<div><span>${esc(r.s.name)}</span><b>${r.current===null?'—':r.current.toFixed(2)}</b></div>`).join('')}</div></div>
 <div class="card"><h3>Detalle individual · ${label}</h3><label>Alumno/a</label><select id="studentDetailSel">${g.students.map(st=>`<option value="${st.id}">${esc(st.name)}</option>`).join('')}</select><div id="studentDetail"></div></div>`
}
function detailHTML(sid){const g=group(),st=g.students.find(x=>x.id===sid);if(!st)return '';const period=state.ui.evaluationPeriod||'eval1',cut=evalCut(period),history=[];criteria.forEach(([c])=>(evidence[c]||[]).forEach((e,i)=>{if(cut&&upIndex(e[0])>upIndex(cut))return;const v=state.scores[scoreKey(g.id,st.id,c,i+1)];if(v!==undefined&&v!==null&&v!==''){const k=scoreKey(g.id,st.id,c,i+1),mark=state.rubricMarks[k];history.push({c,up:e[0],desc:e[2],v,seq:i+1,mark})}}));return `<div class="evaluation-student-metrics"><div><span>Nota ${evalLabel(period)}</span><b>${gradeAt(g.id,st.id,cut)?.toFixed(2)||'—'}</b></div><div><span>Cobertura</span><b>${coverageAt(g.id,st.id,cut).toFixed(0)}%</b></div><div><span>Criterios evaluados</span><b>${assessedCountAt(g.id,st.id,cut)}/19</b></div></div><div class="table-wrap" style="margin-top:10px"><table><thead><tr><th>Criterio</th><th>Peso</th><th>Nivel al cierre</th></tr></thead><tbody>${criteria.map(([c,,w])=>{const v=criterionValueAt(g.id,st.id,c,cut);return `<tr><td>${c}</td><td>${w}%</td><td>${v===null?'—':v.toFixed(2)}</td></tr>`}).join('')}</tbody></table></div><h3 style="margin-top:16px">Evidencias consideradas ●</h3>${history.length?history.map(h=>`<div class="history-item"><b>${h.c} · ${h.up} · ${h.mark?esc(h.mark.label)+' · ':''}${h.v}</b><br>${esc(h.desc)}${h.mark?`<div class="history-rubric">${esc(h.mark.desc)}</div>`:''}</div>`).join(''):`<div class="muted">Todavía no hay evidencias principales registradas para este cierre.</div>`}`}
...
function viewAjustes(){const a=auditData();return `<div class="card"><h2>Grupos y alumnado</h2><div class="row"><input id="newGroup" placeholder="Ej. 1.º ESO A"><button class="btn" id="addGroup">Crear grupo</button></div>${state.groups.map(g=>`<div class="card" style="margin-top:10px"><div class="row"><div><b>${esc(g.name)}</b><div class="tiny">${g.students.length} alumnos</div></div><button class="btn small secondary" data-select-group="${g.id}">Usar</button><button class="btn small danger" data-delete-group="${g.id}">Borrar</button></div><div class="row" style="margin-top:8px"><textarea id="bulk-${g.id}" rows="3" placeholder="Pega un alumno por línea"></textarea><button class="btn small" data-add-students="${g.id}">Añadir</button></div></div>`).join('')}</div>
<div class="card"><h2>Cortes de evaluación</h2><p class="muted">Las evaluaciones son acumulativas. Puedes ajustar qué UP cierra cada trimestre según tu temporalización real.</p><div class="row"><div><label>Fin de 1.ª evaluación</label><select id="firstEvalCut">${ups.map(([u,n])=>`<option value="${u}" ${state.evalConfig.firstCut===u?'selected':''}>${u} · ${esc(n)}</option>`).join('')}</select></div><div><label>Fin de 2.ª evaluación</label><select id="secondEvalCut">${ups.map(([u,n])=>`<option value="${u}" ${state.evalConfig.secondCut===u?'selected':''}>${u} · ${esc(n)}</option>`).join('')}</select></div></div><p class="tiny">Configuración inicial: 1.ª evaluación hasta UP4 y 2.ª hasta UP8. Cámbiala si tu calendario definitivo sitúa los cierres en otras unidades.</p></div>
...
function bind(){document.querySelectorAll('[data-go]').forEach(b=>b.onclick=()=>{currentView=b.dataset.go;render()});document.querySelectorAll('.bottom-nav button').forEach(b=>b.onclick=()=>{currentView=b.dataset.view;render()});
 ...
 document.querySelectorAll('[data-eval-period]').forEach(b=>b.onclick=()=>{state.ui.evaluationPeriod=b.dataset.evalPeriod;save();render()});
 ...
 if(currentView==='resumen'){const sd=document.getElementById('studentDetailSel'),box=document.getElementById('studentDetail');const draw=()=>box.innerHTML=detailHTML(sd.value);if(sd){sd.onchange=draw;draw()}}
 ...
 document.querySelectorAll('[data-select-group]').forEach(b=>b.onclick=()=>{state.selectedGroup=b.dataset.selectGroup;save();render()});document.querySelectorAll('[data-delete-group]').forEach(b=>b.onclick=()=>{if(confirm('¿Borrar este grupo y sus datos?')){const gid=b.dataset.deleteGroup;state.groups=state.groups.filter(g=>g.id!==gid);[state.scores,state.rubricMarks,state.qual,state.attendance,state.notes,state.fitness,state.reflections,state.warmups].forEach(obj=>Object.keys(obj).filter(k=>k.startsWith(gid+'|')).forEach(k=>delete obj[k]));state.selectedGroup=state.groups[0]?.id||null;save();render()}});
...
function exportCSV(){const g=group();if(!g){alert('Selecciona un grupo');return}const period=state.ui.evaluationPeriod||'eval1',cut=evalCut(period),head=['Alumno','Evaluación','Cobertura','Criterios evaluados','CE1','CE2','CE3','CE4','CE5','Nota evaluación','Nivel criterial actual'];const rows=g.students.map(st=>[st.name,evalLabel(period),coverageAt(g.id,st.id,cut),assessedCountAt(g.id,st.id,cut),...['CE1','CE2','CE3','CE4','CE5'].map(ce=>ceValueAt(g.id,st.id,ce,cut)?.toFixed(2)||''),gradeAt(g.id,st.id,cut)?.toFixed(2)||'',finalGrade(g.id,st.id)?.toFixed(2)||'']);download(`resumen-${period}-${g.name}.csv`,[head,...rows].map(r=>r.map(x=>'"'+String(x).replaceAll('"','""')+'"').join(';')).join('\n'),'text/csv;charset=utf-8')}
