...
function defaultState(){return {groups:[],scores:{},rubricMarks:{},qual:{},attendance:{},notes:{},fitness:{},reflections:{},warmups:{},up3RPE:{},up3Tasks:{},up4Game:{},photos:{},selectedGroup:null,selectedUP:'UP1',selectedDate:todayISO(),evalConfig:{firstCut:'UP4',secondCut:'UP10'},ui:{scoreMode:'numeric',evaluationPeriod:'eval1',openWarmup:null,openUp3RPE:null,openUp3Task:null,openUp4Game:null}}}
function migrate(raw){const d=defaultState(),s=Object.assign(d,raw||{});s.attendance=s.attendance||{};s.notes=s.notes||{};s.rubricMarks=s.rubricMarks||{};s.fitness=s.fitness||{};s.reflections=s.reflections||{};s.warmups=s.warmups||{};s.up3RPE=s.up3RPE||{};s.up3Tasks=s.up3Tasks||{};s.up4Game=s.up4Game||{};s.photos=s.photos||{};s.selectedDate=s.selectedDate||todayISO();s.evalConfig=Object.assign({firstCut:'UP4',secondCut:'UP10'},s.evalConfig||{});s.ui=Object.assign({scoreMode:'numeric',evaluationPeriod:'eval1',openWarmup:null,openUp3RPE:null,openUp3Task:null,openUp4Game:null},s.ui||{});return s}
...
function up4GameInstrumentHTML(){
 ...
 const levels=[['Inicio',4],['Básico',6],['Adecuado',8],['Avanzado',10]];
 const indicators=[
...
function bind(){document.querySelectorAll('[data-go]').forEach(b=>b.onclick=()=>{currentView=b.dataset.go;render()});document.querySelectorAll('.bottom-nav button').forEach(b=>b.onclick=()=>{currentView=b.dataset.view;render()});
 ...
 document.querySelectorAll('[data-select-group]').forEach(b=>b.onclick=()=>{state.selectedGroup=b.dataset.selectGroup;save();render()});document.querySelectorAll('[data-delete-group]').forEach(b=>b.onclick=()=>{if(confirm('¿Borrar este grupo y sus datos?')){const gid=b.dataset.deleteGroup;state.groups=state.groups.filter(g=>g.id!==gid);[state.scores,state.rubricMarks,state.qual,state.attendance,state.notes,state.fitness,state.reflections,state.warmups,state.up3RPE,state.up3Tasks,state.up4Game,state.photos].forEach(obj=>Object.keys(obj).filter(k=>k.startsWith(gid+'|')).forEach(k=>delete obj[k]));state.selectedGroup=state.groups[0]?.id||null;save();render()}});
