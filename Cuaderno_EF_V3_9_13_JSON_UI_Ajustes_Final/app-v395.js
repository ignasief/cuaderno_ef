...
const compMatrix={'1.2':['UP2'],'1.6':['UP2','UP6'],'2.1':['UP4'],'2.2':['UP11'],'3.1':['UP3','UP5','UP7','UP8'],'3.2':['UP2','UP3','UP4'],'3.3':['UP1','UP2','UP3'],'4.1':['UP8']};
...
function criterionValueForPeriod(gid,sid,c,period){const items=[];(evidence[c]||[]).forEach((e,i)=>{if(!evidenceInPeriod(c,e[0],period))return;const v=state.scores[scoreKey(gid,sid,c,i+1)];if(v!==undefined&&v!==null&&v!=='')items.push({up:e[0],order:0,v:Number(v)})});Object.entries(state.continuousClosures||{}).forEach(([k,o])=>{const [gg,ss,cc,up]=k.split('|');if(gg!==gid||ss!==sid||cc!==c||!o||o.value===undefined||!evidenceInPeriod(c,up,period))return;items.push({up,order:1,v:Number(o.value)})});items.sort((a,b)=>upIndex(a.up)-upIndex(b.up)||a.order-b.order);return weightedEvidenceValue(items.map(x=>x.v),c)}
...
function summaryStudentHTML(g,st,period,idx){
 ...
 const closureHistory=[],keepDecisionHistory=[];Object.entries(state.continuousClosures||{}).forEach(([k,o])=>{const [gg,ss,c,up]=k.split('|');if(gg!==g.id||ss!==st.id||!o||!evidenceInPeriod(c,up,period))return;if(o.value!==undefined){const item={c,up,desc:'Cierre de evaluación continua confirmado por el profesor',v:Number(o.value).toFixed(2),mark:null,date:o.date||''};history.push(item);closureHistory.push(item)}else if(o.mode==='kept'){keepDecisionHistory.push({c,up,date:o.date||'',maintainedValue:o.maintainedValue,continuousValue:o.continuousValue})}});history.sort((a,b)=>{const ca=parseFloat(a.c)-parseFloat(b.c);if(ca)return ca;const ua=upIndex(a.up)-upIndex(b.up);if(ua)return ua;return a.desc.startsWith('Cierre')?1:-1});
