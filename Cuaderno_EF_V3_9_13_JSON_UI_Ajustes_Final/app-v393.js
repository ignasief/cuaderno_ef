...
function continuousClosureHTML(comps){
 ...
 const rows=comps.map(ev=>{const h=continuousHistory(g.id,sid,ev.c,state.selectedUP),level=continuousLevel(g.id,sid,ev.c,state.selectedUP),ck=closureKey(g.id,sid,ev.c,state.selectedUP),closed=state.continuousClosures?.[ck];const principal=criterionValue(g.id,sid,ev.c);const enough=h.length>=2,canClose=isContinuousCriterion(ev.c);
  return `<div class="closure-row"><div class="closure-head"><b>${criterionLabel(ev.c)}</b>${closed?'<span class="pill closure-done">✓ Cierre confirmado</span>':''}</div>
   <div class="closure-metrics"><span>● Nivel criterial <b>${principal===null?'—':principal.toFixed(2)}</b></span><span>○ Nivel continuo <b>${level===null?'—':level.toFixed(2)}</b></span></div>
   ...
   ${closed?`<div class="tiny">Evidencia ● de cierre: ${Number(closed.value).toFixed(2)} · ${esc(closed.date||'')}</div>`:''}
  </div>`}).join('');
