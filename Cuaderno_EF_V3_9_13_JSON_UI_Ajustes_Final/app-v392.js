...
function continuousClosureHTML(comps){
 ...
 const rows=comps.map(ev=>{const h=continuousHistory(g.id,sid,ev.c,state.selectedUP),level=continuousLevel(g.id,sid,ev.c,state.selectedUP),ck=closureKey(g.id,sid,ev.c,state.selectedUP),closed=state.continuousClosures?.[ck];const principal=criterionValue(g.id,sid,ev.c);const enough=h.length>=2;
...
function summaryStudentHTML(g,st,period,idx){
 ...
 Object.entries(state.continuousClosures||{}).forEach(([k,o])=>{const [gg,ss,c,up]=k.split('|');if(gg===g.id&&ss===st.id&&o&&o.value!==undefined&&evidenceInPeriod(c,up,period))history.push({c,up,desc:'Cierre de evaluación continua confirmado por el profesor',v:Number(o.value).toFixed(2),mark:null})});
