...
const PHOTO_KEY='cuadernoEF_photos_v390';
const criteria=[
 ['1.1','CE1',1],['1.2','CE1',1],['1.3','CE1',1],['1.4','CE1',1],['1.5','CE1',1],['1.6','CE1',1],
 ['2.1','CE2',1],['2.2','CE2',1],['2.3','CE2',1],['2.4','CE2',1],
 ['3.1','CE3',1],['3.2','CE3',1],['3.3','CE3',1],
 ['4.1','CE4',1],['4.2','CE4',1],['4.3','CE4',1],
 ['5.1','CE5',1],['5.2','CE5',1],['5.3','CE5',1]
];
...
const evidence={
'1.1':[['UP1','Revisión de portfolio','Pasaporte de condición física'],['UP3','Revisión de portfolio','Seguimiento y regulación de CFB']],
'1.2':[['UP2','Presentación de producto','Diseño y dirección de calentamiento'],['UP3','Prueba de ejecución','Reto de regulación del esfuerzo'],['UP6','Presentación de producto','Producto aplicado sobre hábitos saludables']],
...
'1.5':[['UP2','Revisión de portfolio','Decálogo del calentamiento: higiene y hábitos posturales'],['UP6','Presentación de producto','Análisis crítico de prácticas y hábitos']],
'1.6':[['UP1','Observación sistemática','Participación responsable y regulación del esfuerzo'],['UP2','Revisión de portfolio','Decálogo y seguimiento de una práctica responsable'],['UP3','Observación sistemática','Participación responsable y autorregulación'],['UP6','Presentación de producto','Aplicación responsable de hábitos saludables']],
...
const compMatrix={'3.1':['UP3','UP4','UP5','UP7','UP8','UP9','UP11','UP13'],'3.2':['UP2','UP3','UP4','UP7','UP10','UP11','UP12','UP13'],'3.3':['UP1','UP2','UP3','UP4','UP7','UP9','UP10','UP11']};
...
function load(){
 try{
  ...
  let split={};
  try{split=JSON.parse(localStorage.getItem(PHOTO_KEY)||'{}')||{}}catch{}
  // Migración única: las fotos antiguas salen del estado principal para que cada pulsación no serialice megabytes.
  if(Object.keys(s.photos||{}).length && !Object.keys(split).length){
   split=s.photos;
   try{localStorage.setItem(PHOTO_KEY,JSON.stringify(split))}catch{}
  }
  s.photos=Object.assign({},split,s.photos||{});
  return s;
 }catch{return defaultState()}
}
let state=load();
function stateWithoutPhotos(){const lite={...state,photos:{}};return lite}
function save(){localStorage.setItem(APP_KEY,JSON.stringify(stateWithoutPhotos()))}
function savePhotos(){localStorage.setItem(PHOTO_KEY,JSON.stringify(state.photos||{}))}
let delayedSaveTimer=null;
function saveSoon(delay=250){clearTimeout(delayedSaveTimer);delayedSaveTimer=setTimeout(()=>{delayedSaveTimer=null;save()},delay)}
...
function invalidateContinuousClosuresForObservationKey(k){
 const [gid,sid,c,up]=String(k||'').split('|');
 if(!gid||!sid||!c||!up||!isContinuousCriterion(c))return 0;
 const changedRank=periodRank(upPeriod(up));
 let removed=0;
 ['eval1','eval2','eval3','final'].forEach(period=>{
  const maxRank=period==='final'?3:periodRank(period);
  if(changedRank>maxRank)return;
  const ck=periodClosureKey(gid,sid,c,period);
  if(state.continuousClosures?.[ck]){delete state.continuousClosures[ck];removed++}
 });
 return removed;
}
...
function pentagonKey(gid,sid){return `${gid}|${sid}`}
const pentagonCaps=[
 ['resistance','Resistencia'],['strength','Fuerza'],['speed','Velocidad'],['mobility','Flexibilidad y movilidad'],['agility','Agilidad']
];
function pentagonValue(gid,sid,phase,cap){const v=state.fitnessPentagon?.[pentagonKey(gid,sid)]?.[phase]?.[cap],n=Number(v);return v===undefined||v===null||v===''||![2,4,6,8,10].includes(n)?null:n}
function pentagonEvolution(a,b){if(a===null||b===null)return '—';return b>a?'Mejora':b<a?'Retroceso':'Mantenimiento'}
function pentagonSvg(gid,sid){
 const cx=150,cy=145,R=104,levels=[2,4,6,8,10];
 const point=(i,val)=>{const ang=-Math.PI/2+i*2*Math.PI/5,r=R*(Number(val)/10);return [cx+Math.cos(ang)*r,cy+Math.sin(ang)*r]};
 const poly=val=>pentagonCaps.map((_,i)=>point(i,val).map(n=>n.toFixed(1)).join(',')).join(' ');
 const dataPoly=phase=>{const vals=pentagonCaps.map(([c])=>pentagonValue(gid,sid,phase,c));if(vals.some(v=>v===null))return '';return vals.map((v,i)=>point(i,v).map(n=>n.toFixed(1)).join(',')).join(' ')};
 const axes=pentagonCaps.map((_,i)=>{const p=point(i,10);return `<line x1="${cx}" y1="${cy}" x2="${p[0]}" y2="${p[1]}" class="pent-axis"/>`}).join('');
 const rings=levels.map(n=>`<polygon points="${poly(n)}" class="pent-ring"/><text x="${cx+3}" y="${cy-R*n/10+11}" class="pent-level">${n}</text>`).join('');
 const labels=pentagonCaps.map(([c,label],i)=>{const p=point(i,12.25),parts=label==='Flexibilidad y movilidad'?['Flexibilidad','y movilidad']:[label];return `<text x="${p[0]}" y="${p[1]}" text-anchor="middle" class="pent-label">${parts.map((x,j)=>`<tspan x="${p[0]}" dy="${j?13:0}">${x}</tspan>`).join('')}</text>`}).join('');
 const ini=dataPoly('initial'),fin=dataPoly('final');
 return `<svg class="fitness-pentagon-svg" viewBox="0 0 300 300" role="img" aria-label="Pentágono de condición física: perfil inicial azul y final rojo">${rings}${axes}${ini?`<polygon points="${ini}" class="pent-profile initial"/>`:''}${fin?`<polygon points="${fin}" class="pent-profile final"/>`:''}${labels}</svg>`;
}
function pentagonTableHTML(gid,sid){return `<div class="table-wrap"><table class="pent-table"><thead><tr><th>Capacidad</th><th>Inicial · UP1</th><th>Final · UP3</th><th>Evolución</th></tr></thead><tbody>${pentagonCaps.map(([c,label])=>{const a=pentagonValue(gid,sid,'initial',c),b=pentagonValue(gid,sid,'final',c),e=pentagonEvolution(a,b);return `<tr><td>${esc(label)}</td><td>${a??'—'}</td><td>${b??'—'}</td><td><span class="pent-evolution ${e==='Mejora'?'improve':e==='Retroceso'?'decline':e==='Mantenimiento'?'same':''}">${e}</span></td></tr>`}).join('')}</tbody></table></div>`}
function pentagonPhaseEditorHTML(g,st,phase){const title=phase==='initial'?'Valoración inicial · UP1':'Valoración final · UP3',k=pentagonKey(g.id,st.id),hasAny=pentagonCaps.some(([c])=>pentagonValue(g.id,st.id,phase,c)!==null);return `<div class="pent-phase"><div class="pent-phase-head"><h4>${title}</h4>${hasAny?`<button type="button" class="btn secondary small pent-delete-phase" data-pent-delete-phase="${phase}" data-pent-key="${k}">Borrar valoración completa</button>`:''}</div>${pentagonCaps.map(([c,label])=>{const v=pentagonValue(g.id,st.id,phase,c);return `<div class="pent-cap-row"><div><b>${esc(label)}</b><small>${v===null?'Vacío':`Nivel ${v}`}</small></div><div class="pent-level-buttons">${[2,4,6,8,10].map(n=>`<button type="button" data-pent-level="${n}" data-pent-key="${k}" data-pent-phase="${phase}" data-pent-cap="${c}" class="${v===n?'selected':''}">${n}</button>`).join('')}<button type="button" class="pent-clear" data-pent-clear="1" data-pent-key="${k}" data-pent-phase="${phase}" data-pent-cap="${c}" ${v===null?'disabled':''}>Borrar</button></div></div>`}).join('')}</div>`}
function pentagonReflectionHTML(g,st,editable=true){const k=pentagonKey(g.id,st.id),v=state.fitnessPentagonReflections[k]||{},fields=[['improved','Capacidad en la que he mejorado'],['work','Capacidad que debo seguir trabajando'],['cause','Posible causa de la evolución'],['challenge','Próximo reto personal']];return `<div class="pent-reflection"><h4>Reflexión final</h4><p class="tiny">Puede utilizarse para fundamentar la evidencia ● del criterio 1.1 en UP3, pero <b>no genera ninguna calificación automáticamente</b>.</p>${fields.map(([f,label])=>`<label><span>${label}</span><textarea rows="2" ${editable?`data-pent-ref-key="${k}" data-pent-ref-field="${f}"`: 'readonly'}>${esc(v[f]??'')}</textarea></label>`).join('')}</div>`}
function fitnessPentagonHTML(g,st,{editable=true,phase=null,showReflection=true}={}){if(!g||!st)return '';return `<section class="fitness-pentagon-card"><div class="pent-head"><div><h3>Mi pentágono de la condición física</h3><p class="muted">Seguimiento exclusivamente intrapersonal. Azul = UP1 inicial · Rojo = UP3 final. El tamaño del pentágono y las marcas físicas no generan nota.</p></div><span class="pill main">Diagnóstico + seguimiento</span></div><div class="pent-layout"><div>${pentagonSvg(g.id,st.id)}<div class="pent-legend"><span><i class="initial"></i>UP1 inicial</span><span><i class="final"></i>UP3 final</span></div></div>${pentagonTableHTML(g.id,st.id)}</div>${editable?(phase?pentagonPhaseEditorHTML(g,st,phase):pentagonPhaseEditorHTML(g,st,'initial')+pentagonPhaseEditorHTML(g,st,'final')):''}${showReflection?pentagonReflectionHTML(g,st,editable):''}</section>`}
function pentagonEvaluateHTML(){const g=fullGroup(),sid=activeEvalSid();if(!g||!sid||!['UP1','UP3'].includes(state.selectedUP))return '';const st=g.students.find(x=>x.id===sid);if(!st)return '';return `<div class="card pent-eval-wrapper">${fitnessPentagonHTML(g,st,{editable:true,phase:state.selectedUP==='UP1'?'initial':'final',showReflection:state.selectedUP==='UP3'})}</div>`}
...
function gradeForPeriod(gid,sid,period){let num=0,den=0;Object.entries(ceWeights).forEach(([ce,w])=>{const v=ceValueForPeriod(gid,sid,ce,period);if(v!==null){num+=v*w;den+=w}});return den?num/den:null}
function evalRangeText(period){if(period==='final')return 'Acumulativa: todas las evidencias del curso, independientemente del orden real de las UP.';const list=periodUps(period);const names=list.map(u=>`${u} · ${upName(u)}`).join(' · ');if(period==='eval2'||period==='eval3')return `${names||'Sin UP asignadas'} · CE3 (3.1, 3.2 y 3.3) mantiene trayectoria continua desde el inicio de curso.`;return names||'Sin UP asignadas a este periodo'}
...
function upHasContinuousObservations(up){return Object.entries(state.qual||{}).some(([k,v])=>{const parts=k.split('|');return parts.length>=4&&parts[3]===up&&v!==undefined&&v!==null&&v!==''})}
function invalidateClosuresForPeriodMove(up,oldPeriod,newPeriod){const a=periodRank(oldPeriod),b=periodRank(newPeriod);if(a===b)return 0;const lo=Math.min(a,b),hi=Math.max(a,b),affected=new Set();Object.keys(state.qual||{}).forEach(k=>{const [gid,sid,c,u]=k.split('|');if(u!==up||!isContinuousCriterion(c))return;for(let rank=lo;rank<hi;rank++){const period=rank===1?'eval1':rank===2?'eval2':'eval3';affected.add(periodClosureKey(gid,sid,c,period))}});let removed=0;affected.forEach(k=>{if(state.continuousClosures?.[k]){delete state.continuousClosures[k];removed++}});return removed}
...
function render(){const app=document.getElementById('app');const fn={inicio:viewInicio,sesion:viewSesion,evaluar:viewEvaluar,resumen:viewResumen,ajustes:viewAjustes}[currentView];try{const html=fn();document.querySelectorAll('.bottom-nav button').forEach(b=>b.classList.toggle('active',b.dataset.view===currentView));app.innerHTML=html;bind()}catch(err){console.error('Cuaderno EF render error',err);app.innerHTML='<div class="card"><h2>Error de pantalla</h2><p class="muted">No se ha podido cargar esta vista. Recarga la aplicación. Los datos guardados no se han borrado.</p></div>';}}
function studentUPComplete(gid,sid,up){
 const principals=principalForUP(up),continuous=compsForUP(up);
 const principalDone=principals.every(ev=>{const v=state.scores[scoreKey(gid,sid,ev.c,ev.seq)];return v!==undefined&&v!==null&&v!==''});
 const continuousDone=continuous.every(ev=>{const v=state.qual[qualKey(gid,sid,ev.c,ev.up)];return v!==undefined&&v!==null&&v!==''});
 return (principals.length||continuous.length)?principalDone&&continuousDone:false
}
function pendingStudentsForUP(g,up){return g?g.students.filter(st=>!studentUPComplete(g.id,st.id,up)).length:0}
function pendingClosuresForPeriod(g,period){if(!g)return 0;let n=0;g.students.forEach(st=>['3.1','3.2','3.3'].forEach(c=>{const h=continuousHistoryForPeriod(g.id,st.id,c,period),key=periodClosureKey(g.id,st.id,c,period);if(h.length>=2&&!state.continuousClosures?.[key])n++}));return n}
function backupDateText(){const raw=state.ui?.lastBackupExport;if(!raw)return 'Todavía no registrada';const d=new Date(raw);return Number.isNaN(d.getTime())?'Todavía no registrada':d.toLocaleString('es-ES',{dateStyle:'short',timeStyle:'short'})}
function viewInicio(){const g=group(),n=g?g.students.length:0,pending=g?pendingStudentsForUP(g,state.selectedUP):0,period=state.ui.evaluationPeriod||'eval1',pendingClosures=pendingClosuresForPeriod(g,period);return `<div class="card"><h2>Panel del curso</h2><div class="grid"><div class="metric"><span>Grupo</span><strong>${g?esc(g.name):'—'}</strong></div><div class="metric"><span>Alumnado</span><strong>${n}</strong></div><div class="metric"><span>UP actual</span><strong>${state.selectedUP}</strong></div><div class="metric"><span>Hoy</span><strong>${state.selectedDate.slice(8,10)}/${state.selectedDate.slice(5,7)}</strong></div><div class="metric"><span>Pendientes · ${state.selectedUP}</span><strong>${g?pending:'—'}</strong></div><div class="metric"><span>Cierres CE3 pendientes</span><strong>${g?pendingClosures:'—'}</strong></div></div></div>
...
function quickEvaluationHTML(evs){
 ...
 ${ctx?`<div class="quick-context"><b>${esc(ctx.title)}</b><span>${esc(ctx.what)}</span></div>`:''}${criterionMeans.length?`<div class="v4-means">${criterionMeans.map(x=>`<span><b>${x.c}</b> media ${x.v===null?'—':x.v.toFixed(2)}</span>`).join('')}</div>`:''}
...
function continuousEvaluationHTML(comps){
 ...
 const rows=comps.map(ev=>{const k=qualKey(g.id,sid,ev.c,ev.up),v=state.qual[k],done=v!==undefined&&v!==null&&v!=='';
  return `<div class="quick-eval-row continuous-row ce-block-3 ${done?'done':''}">
   <div class="quick-eval-head"><div><span class="pill continuous ce-pill ce-block-3">○ ${criterionLabel(ev.c)}</span><b class="quick-focus">${esc(quickFocus(ev.up,ev.c,criterionShort[ev.c]||''))}</b></div><span class="quick-status continuous-status">${done?`✓ ${labels[Number(v)]||''} · ${v}`:'Pendiente'}</span></div>
...
function continuousClosureHTML(comps){
 const g=fullGroup(),sid=activeEvalSid();if(!g||!sid||!comps.length)return '';
 const rows=comps.map(ev=>{const h=continuousHistory(g.id,sid,ev.c,state.selectedUP),level=continuousLevel(g.id,sid,ev.c,state.selectedUP),ck=closureKey(g.id,sid,ev.c,state.selectedUP),closed=state.continuousClosures?.[ck];const period=state.ui.evaluationPeriod||'eval1';const principal=criterionValueForPeriod(g.id,sid,ev.c,period);const enough=h.length>=2,canClose=isContinuousCriterion(ev.c);const kept=closed?.mode==='kept';
  const actionHtml=closed?(kept?`<div class="closure-confirmed-box"><div class="tiny">Decisión docente: mantener nivel criterial${closed.maintainedValue!==null&&closed.maintainedValue!==undefined?` · ${Number(closed.maintainedValue).toFixed(2)}`:''} · ${esc(closed.date||'')}</div><button type="button" class="btn secondary small" data-closure-reopen="${ck}" data-closure-c="${ev.c}">Reabrir cierre</button></div>`:`<div class="closure-confirmed-box"><div class="tiny">Evidencia ● de cierre: ${Number(closed.value).toFixed(2)} · ${esc(closed.date||'')}</div><button type="button" class="btn secondary small" data-closure-reopen="${ck}" data-closure-c="${ev.c}">Reabrir cierre</button></div>`):!canClose?`<div class="closure-warning">Seguimiento formativo: este criterio ○ no genera actualización de calificación al cierre.</div>`:!enough?`<div class="closure-warning">Seguimiento insuficiente para propuesta de actualización. Se necesitan al menos 2 observaciones ○.</div>`:`<div class="closure-actions"><button type="button" class="btn secondary small" data-closure-keep="${ck}" data-closure-c="${ev.c}" data-closure-principal="${principal===null?'':principal.toFixed(4)}" data-closure-continuous="${level===null?'':level.toFixed(4)}">Mantener nivel actual</button><button type="button" class="btn small" data-closure-confirm="${ck}" data-closure-value="${level.toFixed(4)}" data-closure-c="${ev.c}">Confirmar actualización · ${level.toFixed(2)}</button></div>`;
  const status=closed?(kept?'<span class="pill closure-done">✓ Cierre revisado · nivel mantenido</span>':'<span class="pill closure-done">✓ Cierre confirmado</span>'):'';
  return `<div class="closure-row"><div class="closure-head"><b>${criterionLabel(ev.c)}</b>${status}</div>
   <div class="closure-metrics"><span>● Nivel criterial (${evalLabel(period)}) <b>${principal===null?'—':principal.toFixed(2)}</b></span><span>○ Nivel continuo <b>${level===null?'—':level.toFixed(2)}</b></span></div>
   <div class="closure-evolution">Evolución ○: ${h.length?h.map(x=>`${x.up} · ${x.value}`).join(' → '):'Sin seguimiento'}</div>
   ${actionHtml}
  </div>`}).join('');
 return `<div class="card continuous-closure-card"><div class="quick-eval-title"><div><h2>Cierre de evaluación continua</h2><p>Al finalizar la UP, revisa el seguimiento ○. Solo una confirmación de actualización crea una nueva evidencia ●; mantener el nivel registra la decisión docente sin cambiar la calificación.</p></div></div><div class="quick-context closure-context"><b>Cierre de ${state.selectedUP}</b><span>El seguimiento no modifica la nota durante la unidad. Con 2 o más observaciones, puedes mantener el nivel o confirmar la propuesta.</span></div>${rows}</div>`;
}
...
const up13Sessions={
 1:{title:'Sesión 1 · Calculamos y planificamos',activities:['Planificación de la ruta'],continuous:[]},
 2:{title:'Sesión 2 · Mochila y prevención',activities:['Preparación razonada de la mochila'],continuous:[]},
 3:{title:'Sesión 3 · Nos preparamos para caminar',activities:['Regulación, cooperación y seguridad'],continuous:['3.1','3.2']},
 4:{title:'Sesión 4 · Salida real',activities:['Realización de la ruta'],continuous:['3.1','3.2']}
};
const up13ActivityContinuous={
 'Planificación de la ruta':[],
 'Preparación razonada de la mochila':[],
 'Cuaderno «Mi progreso»':[],
...
function formalCE3ClosureHTML(g,st,period){
 ...
 const rows=['3.1','3.2','3.3'].map(c=>{const h=continuousHistoryForPeriod(g.id,st.id,c,period),level=continuousLevelForPeriod(g.id,st.id,c,period),key=periodClosureKey(g.id,st.id,c,period),closed=state.continuousClosures?.[key],principal=criterionValueForPeriod(g.id,st.id,c,period),enough=h.length>=2,kept=closed?.mode==='kept';const status=closed?(kept?'<span class="pill closure-done">✓ Revisado · nivel mantenido</span>':'<span class="pill closure-done">✓ Cierre confirmado</span>'):'';const action=closed?`<div class="closure-confirmed-box"><div class="tiny">${kept?'Decisión docente: mantener el nivel':'Nivel continuo incorporado como evidencia ●: '+Number(closed.value).toFixed(2)} · ${esc(closed.date||'')}</div><button type="button" class="btn secondary small" data-closure-reopen="${key}" data-closure-c="${c}">Reabrir cierre</button></div>`:!enough?'<div class="closure-warning">Seguimiento insuficiente: se necesitan al menos 2 observaciones ○ para cerrar el criterio.</div>':`<div class="closure-actions"><button type="button" class="btn secondary small" data-closure-keep="${key}" data-closure-period="${period}" data-closure-c="${c}" data-closure-principal="${principal===null?'':principal.toFixed(4)}" data-closure-continuous="${level.toFixed(4)}">Mantener nivel actual</button><button type="button" class="btn small" data-closure-confirm="${key}" data-closure-period="${period}" data-closure-value="${level.toFixed(4)}" data-closure-c="${c}">Confirmar cierre · ${level.toFixed(2)}</button></div>`;return `<div class="closure-row"><div class="closure-head"><b>${criterionLabel(c)}</b>${status}</div><div class="closure-metrics"><span>● Nivel criterial (${evalLabel(period)}) <b>${principal===null?'—':principal.toFixed(2)}</b></span><span>○ Nivel continuo acumulado <b>${level===null?'—':level.toFixed(2)}</b></span></div><div class="closure-evolution">Trayectoria ○: ${h.length?h.map(x=>`${x.up} · ${x.value}`).join(' → '):'Sin seguimiento'}</div>${action}</div>`}).join('');return `<details class="criteria-detail" open><summary>Cierre formal CE3 · ${evalLabel(period)}</summary><p class="muted">Las observaciones ○ se acumulan desde el inicio de curso. El cálculo prioriza las tres más recientes: 15% · 25% · 60% (con dos: 29,4% · 70,6%). El cierre actual es la referencia continua de esta evaluación; no se promedian cierres anteriores.</p>${rows}</details>`}
function summaryStudentHTML(g,st,period,idx){
 ...
 const closureHistory=[],keepDecisionHistory=[];Object.entries(state.continuousClosures||{}).forEach(([k,o])=>{const [gg,ss,c,tag]=k.split('|');if(gg!==g.id||ss!==st.id||!o||tag!==period)return;if(o.value!==undefined){const item={c,up:evalLabel(period),desc:'Cierre formal de evaluación continua confirmado por el profesor',v:Number(o.value).toFixed(2),mark:null,date:o.date||''};history.push(item);closureHistory.push(item)}else if(o.mode==='kept'){keepDecisionHistory.push({c,up:evalLabel(period),date:o.date||'',maintainedValue:o.maintainedValue,continuousValue:o.continuousValue})}});history.sort((a,b)=>{const ca=parseFloat(a.c)-parseFloat(b.c);if(ca)return ca;const ua=upIndex(a.up)-upIndex(b.up);if(ua)return ua;return a.desc.startsWith('Cierre')?1:-1});
 ...
 <div class="evaluation-student-metrics"><div><span>Nota ${evalLabel(period)}</span><b>${grade===null?'—':grade.toFixed(2)}</b>${grade===null&&period!=='final'?`<small>Sin evidencias en esta evaluación</small>`:''}</div><div><span>Peso evaluado</span><b>${cov.toFixed(0)}%</b></div><div><span>Criterios evaluados</span><b>${count}/19</b></div></div>
 ...
 ${fitnessPentagonHTML(g,st,{editable:true})}
 ${formalCE3ClosureHTML(g,st,period)}
 <details class="criteria-detail"><summary>Ver detalle de criterios</summary><div class="table-wrap"><table><thead><tr><th>Criterio</th><th>Peso</th><th>Nivel</th></tr></thead><tbody>${criteria.map(([c,,w])=>{const v=criterionValueForPeriod(g.id,st.id,c,period);const cls=closureHistory.filter(h=>h.c===c);return `<tr><td>${criterionLabel(c)}${cls.length?`<div class="tiny">● Cierre continuo: ${cls.map(h=>`${h.up} · ${h.v}`).join(' · ')}</div>`:''}</td><td>${w}</td><td>${v===null?'—':v.toFixed(2)}</td></tr>`}).join('')}</tbody></table></div></details>
 ${closureHistory.length?`<details class="criteria-detail" open><summary>Cierres de evaluación continua ● (${closureHistory.length})</summary>${closureHistory.map(h=>`<div class="history-item closure-history-item"><b>${criterionLabel(h.c)} · ${h.up} · ${h.v}</b><br>Cierre de evaluación continua confirmado${h.date?` · ${esc(h.date)}`:''}</div>`).join('')}</details>`:''}${keepDecisionHistory.length?`<details class="criteria-detail" open><summary>Decisiones de cierre sin cambio de nota (${keepDecisionHistory.length})</summary>${keepDecisionHistory.map(h=>`<div class="history-item"><b>${criterionLabel(h.c)} · ${h.up} · nivel mantenido${h.maintainedValue!==null&&h.maintainedValue!==undefined?` ${Number(h.maintainedValue).toFixed(2)}`:''}</b><br>Revisión docente: se mantiene el nivel criterial${h.continuousValue!==null&&h.continuousValue!==undefined?` · propuesta ○ ${Number(h.continuousValue).toFixed(2)}`:''}${h.date?` · ${esc(h.date)}`:''}</div>`).join('')}</details>`:''}<details class="criteria-detail"><summary>Evidencias consideradas ● (${history.length})</summary>${history.length?history.map(h=>`<div class="history-item"><b>${criterionLabel(h.c)} · ${h.up} · ${h.mark?esc(h.mark.label)+' · ':''}${h.v}</b><br>${esc(h.desc)}${h.mark?`<div class="history-rubric">${esc(h.mark.desc)}</div>`:''}</div>`).join(''):`<div class="muted summary-empty">Sin evidencias principales registradas en esta evaluación.</div>`}</details>
 </div></details>`
}
function viewResumen(){
 ...
 return `${groupSelectors()}
 <div class="card evaluation-panel"><div class="evaluation-head"><div><h2>Evaluaciones</h2><p class="muted">CE3 (3.1, 3.2 y 3.3) se observa durante las UP y se cierra formalmente al final de cada evaluación. La trayectoria es acumulativa y prioriza lo reciente (15% · 25% · 60%). Cada evaluación obtiene su propio cierre; los cierres anteriores no se vuelven a promediar.</p></div></div>
 <div class="evaluation-tabs"><button data-eval-period="eval1" class="${period==='eval1'?'active':''}">1.ª evaluación</button><button data-eval-period="eval2" class="${period==='eval2'?'active':''}">2.ª evaluación</button><button data-eval-period="eval3" class="${period==='eval3'?'active':''}">3.ª evaluación</button><button data-eval-period="final" class="${period==='final'?'active':''}">Final</button></div>
...
function auditData(){const sums={};criteria.forEach(([c,ce])=>{const es=evidence[c]||[],n=es.length;if(!n)return;const nCrit=criteria.filter(x=>x[1]===ce).length,criterionPct=ceWeights[ce]/nCrit,share=criterionPct/n;es.forEach(e=>{sums[e[1]]=(sums[e[1]]||0)+share})});return sums}
...
function updateProgressForCard(card){
 if(!card)return;
 const rows=[...card.querySelectorAll('.quick-eval-row')];
 const done=rows.filter(r=>r.classList.contains('done')).length;
 const p=card.querySelector('.quick-progress');if(p){p.textContent=`${done}/${rows.length}`;p.classList.toggle('complete',rows.length>0&&done===rows.length)}
 const txt=card.querySelector('.quick-eval-title p');if(txt){txt.textContent=card.classList.contains('continuous-eval-card')?`${done}/${rows.length} seguimientos registrados · guardado automático`:`${done}/${rows.length} criterios registrados · guardado automático`}
}
function ensureClearButton(row,key,attr){
 let clear=row.querySelector(`[${attr}]`);if(clear)return;
 clear=document.createElement('button');clear.type='button';clear.className='quick-clear';clear.setAttribute(attr,key);clear.textContent='Borrar';
 row.appendChild(clear);
 clear.onclick=()=>{if(attr==='data-quick-clear'){delete state.scores[key];delete state.rubricMarks[key]}else delete state.qual[key];save();render()};
}
function updateQuickRowUI(btn,score,label){
 const row=btn.closest('.quick-eval-row');if(!row)return;
 row.querySelectorAll('[data-quick-score]').forEach(x=>x.classList.toggle('selected',x===btn));row.classList.add('done');
 const st=row.querySelector('.quick-status');if(st)st.textContent=`✓ ${label?label+' · ':''}${score}`;
 ensureClearButton(row,btn.dataset.quickKey,'data-quick-clear');updateProgressForCard(row.closest('.quick-eval-card'));
}
function updateContinuousRowUI(btn,score){
 const labels={2:'No adquirido',4:'En proceso',6:'Básico',8:'Adecuado',10:'Consolidado'};
 const row=btn.closest('.quick-eval-row');if(!row)return;
 row.querySelectorAll('[data-cont-score]').forEach(x=>x.classList.toggle('selected',x===btn));row.classList.add('done');
 const st=row.querySelector('.quick-status');if(st)st.textContent=`✓ ${labels[score]||''} · ${score}`;
...
function bind(){document.querySelectorAll('[data-go]').forEach(b=>b.onclick=()=>{currentView=b.dataset.go;render()});document.querySelectorAll('.bottom-nav button').forEach(b=>b.onclick=()=>{currentView=b.dataset.view;render()});
 ...
 document.querySelectorAll('[data-closure-reopen]').forEach(b=>b.onclick=()=>{const key=b.dataset.closureReopen,closed=state.continuousClosures?.[key];if(!closed)return;const kept=closed.mode==='kept';const msg=kept?`Vas a reabrir este cierre de evaluación continua. La decisión docente de mantener el nivel dejará de constar como cierre hasta que vuelvas a decidir. La calificación no se modificará. ¿Reabrir cierre?`:`Vas a reabrir este cierre de evaluación continua. La evidencia ● de cierre (${Number(closed.value).toFixed(2)}) dejará de contar en la calificación hasta que vuelvas a confirmarlo. ¿Reabrir cierre?`;const ok=confirm(msg);if(!ok)return;delete state.continuousClosures[key];save();toast('Cierre reabierto');render()});
 document.querySelectorAll('[data-qual-key]').forEach(q=>q.onchange=()=>{const k=q.dataset.qualKey,prev=state.qual[k]??'';if(q.value)state.qual[k]=q.value;else delete state.qual[k];if(String(prev)!==String(q.value||''))invalidateContinuousClosuresForObservationKey(k);save();toast('Seguimiento guardado')});
 document.querySelectorAll('[data-teacher-note]').forEach(t=>t.oninput=()=>{const v=t.value.slice(0,500);if(v)state.teacherNotes[t.dataset.teacherNote]=v;else delete state.teacherNotes[t.dataset.teacherNote];const c=document.querySelector(`[data-teacher-count="${CSS.escape(t.dataset.teacherNote)}"]`);if(c)c.textContent=`${v.length}/500`;saveSoon(300)});
 ...
 document.querySelectorAll('[data-up-period]').forEach(sel=>sel.onchange=()=>{const up=sel.dataset.upPeriod,prev=upPeriod(up),next=sel.value;if(prev===next)return;const hasPrincipal=upHasPrincipalScores(up),hasContinuous=upHasContinuousObservations(up);if(hasPrincipal||hasContinuous){const ok=confirm(`${up} ya contiene datos de evaluación. Cambiarla de evaluación modificará los cálculos del periodo${hasContinuous?' y puede reabrir cierres CE3 afectados':''}. ¿Quieres continuar?`);if(!ok){sel.value=prev;return}}state.evalConfig.upPeriods[up]=next;const reopened=invalidateClosuresForPeriodMove(up,prev,next);save();toast(`${up} asignada a ${evalLabel(next)}${reopened?` · ${reopened} cierre(s) CE3 reabierto(s)`:''}`);render()});
 document.querySelectorAll('[data-pent-level]').forEach(b=>b.onclick=()=>{const k=b.dataset.pentKey,phase=b.dataset.pentPhase,cap=b.dataset.pentCap,n=Number(b.dataset.pentLevel);state.fitnessPentagon[k]=state.fitnessPentagon[k]||{};state.fitnessPentagon[k][phase]=state.fitnessPentagon[k][phase]||{};const cur=state.fitnessPentagon[k][phase][cap];if(cur!==undefined&&Number(cur)===n)delete state.fitnessPentagon[k][phase][cap];else state.fitnessPentagon[k][phase][cap]=n;save();render();toast(cur!==undefined&&Number(cur)===n?'Registro borrado · vacío':`Pentágono · nivel ${n}`)});
 document.querySelectorAll('[data-pent-clear]').forEach(b=>b.onclick=()=>{const k=b.dataset.pentKey,phase=b.dataset.pentPhase,cap=b.dataset.pentCap;if(state.fitnessPentagon?.[k]?.[phase]){delete state.fitnessPentagon[k][phase][cap];if(!Object.keys(state.fitnessPentagon[k][phase]).length)delete state.fitnessPentagon[k][phase];if(!Object.keys(state.fitnessPentagon[k]).length)delete state.fitnessPentagon[k]}save();render();toast('Registro borrado · vacío')});
 document.querySelectorAll('[data-pent-delete-phase]').forEach(b=>b.onclick=()=>{const k=b.dataset.pentKey,phase=b.dataset.pentDeletePhase,label=phase==='initial'?'inicial de UP1':'final de UP3';if(!state.fitnessPentagon?.[k]?.[phase])return;if(!confirm(`¿Borrar la valoración ${label} completa? Los cinco vértices quedarán vacíos.`))return;delete state.fitnessPentagon[k][phase];if(!Object.keys(state.fitnessPentagon[k]).length)delete state.fitnessPentagon[k];save();render();toast('Valoración borrada · valores vacíos')});
 document.querySelectorAll('[data-pent-ref-field]').forEach(el=>{const store=()=>{const k=el.dataset.pentRefKey,f=el.dataset.pentRefField;state.fitnessPentagonReflections[k]=state.fitnessPentagonReflections[k]||{};const val=el.value.trim();if(val)state.fitnessPentagonReflections[k][f]=val;else delete state.fitnessPentagonReflections[k][f];save()};el.oninput=store;el.onchange=()=>{store();toast('Reflexión guardada')}});
...
function download(name,text,type){const blob=new Blob([text],{type});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=name;a.style.display='none';document.body.appendChild(a);a.click();setTimeout(()=>{a.remove();URL.revokeObjectURL(url)},2000)}
