...
function continuousClosureHTML(comps){
 ...
 const rows=comps.map(ev=>{const h=continuousHistory(g.id,sid,ev.c,state.selectedUP),level=continuousLevel(g.id,sid,ev.c,state.selectedUP),ck=closureKey(g.id,sid,ev.c,state.selectedUP),closed=state.continuousClosures?.[ck];const period=state.ui.evaluationPeriod||'eval1';const principal=criterionValueForPeriod(g.id,sid,ev.c,period);const enough=h.length>=2,canClose=isContinuousCriterion(ev.c);
  const actionHtml=closed?`<div class="closure-confirmed-box"><div class="tiny">Evidencia ● de cierre: ${Number(closed.value).toFixed(2)} · ${esc(closed.date||'')}</div><button type="button" class="btn secondary small" data-closure-reopen="${ck}" data-closure-c="${ev.c}">Reabrir cierre</button></div>`:!canClose?`<div class="closure-warning">Seguimiento formativo: este criterio ○ no genera actualización de calificación al cierre.</div>`:!enough?`<div class="closure-warning">Seguimiento insuficiente para propuesta de actualización. Se necesitan al menos 2 observaciones ○.</div>`:`<div class="closure-actions"><button type="button" class="btn secondary small" data-closure-keep="${ck}">Mantener nivel actual</button><button type="button" class="btn small" data-closure-confirm="${ck}" data-closure-value="${level.toFixed(4)}" data-closure-c="${ev.c}">Confirmar actualización · ${level.toFixed(2)}</button></div>`;
  return `<div class="closure-row"><div class="closure-head"><b>${criterionLabel(ev.c)}</b>${closed?'<span class="pill closure-done">✓ Cierre confirmado</span>':''}</div>
 ...
 return `<div class="card continuous-closure-card"><div class="quick-eval-title"><div><h2>Cierre de evaluación continua</h2><p>Al finalizar la UP, revisa el seguimiento ○. Solo una confirmación del profesor crea una nueva evidencia ●.</p></div></div><div class="quick-context closure-context"><b>Cierre de ${state.selectedUP}</b><span>El seguimiento no modifica la nota durante la unidad. Con 2 o más observaciones, puedes mantener el nivel o confirmar la propuesta.</span></div>${rows}</div>`;
}
...
function summaryStudentHTML(g,st,period,idx){
 ...
 const closureHistory=[];Object.entries(state.continuousClosures||{}).forEach(([k,o])=>{const [gg,ss,c,up]=k.split('|');if(gg===g.id&&ss===st.id&&o&&o.value!==undefined&&evidenceInPeriod(c,up,period)){const item={c,up,desc:'Cierre de evaluación continua confirmado por el profesor',v:Number(o.value).toFixed(2),mark:null,date:o.date||''};history.push(item);closureHistory.push(item)}});history.sort((a,b)=>{const ca=parseFloat(a.c)-parseFloat(b.c);if(ca)return ca;const ua=upIndex(a.up)-upIndex(b.up);if(ua)return ua;return a.desc.startsWith('Cierre')?1:-1});
 ...
 ${closureHistory.length?`<details class="criteria-detail" open><summary>Cierres de evaluación continua ● (${closureHistory.length})</summary>${closureHistory.map(h=>`<div class="history-item closure-history-item"><b>${criterionLabel(h.c)} · ${h.up} · ${h.v}</b><br>Cierre de evaluación continua confirmado${h.date?` · ${esc(h.date)}`:''}</div>`).join('')}</details>`:''}<details class="criteria-detail"><summary>Evidencias consideradas ● (${history.length})</summary>${history.length?history.map(h=>`<div class="history-item"><b>${criterionLabel(h.c)} · ${h.up} · ${h.mark?esc(h.mark.label)+' · ':''}${h.v}</b><br>${esc(h.desc)}${h.mark?`<div class="history-rubric">${esc(h.mark.desc)}</div>`:''}</div>`).join(''):`<div class="muted summary-empty">Sin evidencias principales registradas en esta evaluación.</div>`}</details>
 </div></details>`
}
...
function bind(){document.querySelectorAll('[data-go]').forEach(b=>b.onclick=()=>{currentView=b.dataset.go;render()});document.querySelectorAll('.bottom-nav button').forEach(b=>b.onclick=()=>{currentView=b.dataset.view;render()});
 ...
 document.querySelectorAll('[data-closure-keep]').forEach(b=>b.onclick=()=>toast('Se mantiene el nivel criterial actual'));
 document.querySelectorAll('[data-closure-confirm]').forEach(b=>b.onclick=()=>{const key=b.dataset.closureConfirm,value=Number(b.dataset.closureValue),c=b.dataset.closureC;if(!Number.isFinite(value)||state.continuousClosures?.[key])return;state.continuousClosures[key]={value,date:todayISO(),criterion:c,up:state.selectedUP,source:'Cierre de evaluación continua',kind:'principal',confirmed:true};save();toast('Actualización confirmada como evidencia ●');render()});
 document.querySelectorAll('[data-closure-reopen]').forEach(b=>b.onclick=()=>{const key=b.dataset.closureReopen,closed=state.continuousClosures?.[key];if(!closed)return;const ok=confirm(`Vas a reabrir este cierre de evaluación continua. La evidencia ● de cierre (${Number(closed.value).toFixed(2)}) dejará de contar en la calificación hasta que vuelvas a confirmarlo. ¿Reabrir cierre?`);if(!ok)return;delete state.continuousClosures[key];save();toast('Cierre reabierto');render()});
