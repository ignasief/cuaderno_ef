...
function pendingClosuresForPeriod(g,period){if(!g||period==='final')return 0;let n=0;g.students.forEach(st=>['3.1','3.2','3.3'].forEach(c=>{const h=continuousHistoryForPeriod(g.id,st.id,c,period),key=periodClosureKey(g.id,st.id,c,period);if(h.length>=2&&!state.continuousClosures?.[key])n++}));return n}
