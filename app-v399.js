...
function bind(){document.querySelectorAll('[data-go]').forEach(b=>b.onclick=()=>{currentView=b.dataset.go;render()});document.querySelectorAll('.bottom-nav button').forEach(b=>b.onclick=()=>{currentView=b.dataset.view;render()});
 ...
 document.querySelectorAll('[data-cont-score]').forEach(b=>b.onclick=()=>{const score=Number(b.dataset.contScore),k=b.dataset.contKey;if(state.qual[k]!==undefined&&state.qual[k]!==null&&state.qual[k]!==''&&Number(state.qual[k])===score){delete state.qual[k];delete state.qualDates[k];save();toast('Evaluación continua · vacío');render();return}state.qual[k]=score;state.qualDates[k]=todayISO();save();updateContinuousRowUI(b,score);toast('Evaluación continua guardada')});
 document.querySelectorAll('[data-cont-clear]').forEach(b=>b.onclick=()=>{delete state.qual[b.dataset.contClear];delete state.qualDates[b.dataset.contClear];save();toast('Seguimiento borrado');render()});
